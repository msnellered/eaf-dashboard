...
try:
    ...
    print(
        "WARNING: numpy_financial package is required for accurate financial calculations. Please install it with: pip install numpy-financial"
    )
        ...
        def irr(self, values):
            ...
            for _ in range(max_iter):
                ...
                try:
                    ...
                    npv_mid = float("inf") if values[0] < 0 else float("-inf")
...
def fill_tou_gaps(periods):
    ...
    for i in range(len(clean_periods) - 1):
        if clean_periods[i][1] > clean_periods[i + 1][0]:
            print(
                f"Warning: Overlapping TOU periods detected between {clean_periods[i]} and {clean_periods[i+1]}."
            )
    # Fill gaps
    filled_periods = []
    current_time = 0.0
    for start, end, rate in clean_periods:
        if start > current_time:
            filled_periods.append((current_time, start, "off_peak"))  # Fill gap before
        filled_periods.append((start, end, rate))
        current_time = end
    if current_time < 24.0:
        filled_periods.append((current_time, 24.0, "off_peak"))  # Fill gap after
    if not filled_periods:
        filled_periods.append((0.0, 24.0, "off_peak"))  # Handle empty case
...
def calculate_eaf_profile(time_minutes, eaf_size=100, cycle_duration=36):
    ...
    for i, t_actual in enumerate(time_minutes):
        ...
        if t_norm_actual_cycle <= bore_in_end_frac:
            ...
            eaf_power[i] = (
                15 + (25 - 15) * phase_progress
            ) * scale  # Ramp up during bore-in
        elif t_norm_actual_cycle <= main_melting_end_frac:
            eaf_power[i] = (
                55 + 5 * np.sin(t_actual * 0.5 * freq_scale)
            ) * scale  # Main melting with variation
            ...
            eaf_power[i] = (
                50 - (50 - 40) * phase_progress
            ) * scale  # Ramp down slightly
        else:  # Refining phase
            eaf_power[i] = (
                20 + 5 * np.sin(t_actual * 0.3 * freq_scale)
            ) * scale  # Lower power refining with variation
    return eaf_power
...
def create_monthly_bill_with_bess(
    ...
    energy_rates = {
        r: rate * seasonal_mult
        for r, rate in utility_params.get(KEY_ENERGY_RATES, {}).items()
    }
    ...
    cycle_duration_min = eaf_params.get(
        KEY_CYCLE_DURATION_INPUT, eaf_params.get(KEY_CYCLE_DURATION, 36)
    )
    if cycle_duration_min <= 0:
        cycle_duration_min = 36  # Fallback
    ...
    grid_cap = eaf_params.get(KEY_GRID_CAP, 50)
    bess_power_max = bess_params.get(KEY_POWER_MAX, 20)
    ...
    return {
        ...
        "bess_discharged_per_cycle_mwh": bess_energy_cycle_discharged,
    }
...
def calculate_annual_billings(eaf_params, bess_params, utility_params):
    ...
    if KEY_TOU_FILLED not in utility_params or not utility_params[KEY_TOU_FILLED]:
        raw_periods = utility_params.get(
            KEY_TOU_RAW, default_utility_params[KEY_TOU_RAW]
        )
...
def calculate_incentives(bess_params, incentive_params):
    ...
    # Helper function already handles None for capacity/power inside it
    total_cost = calculate_initial_bess_cost(bess_params)

    # --- ADDED: Handle NoneType for capacity before calculating kWh ---
    capacity_mwh = bess_params.get(KEY_CAPACITY, 0)
    capacity_mwh = capacity_mwh if capacity_mwh is not None else 0
    # --- End Add ---
    capacity_kwh = capacity_mwh * 1000
...
def calculate_financial_metrics(
    ...
    print(
        f"DEBUG: BESS parameters used: cycle_life={bess_params.get(KEY_CYCLE_LIFE)}, rte={bess_params.get(KEY_RTE)}"
    )
    ...
    om_cost_per_kwh_yr = bess_params.get(
        KEY_OM_KWHR_YR, None
    )  # Keep None possibility here
    ...
    insurance_percent_yr = (
        insurance_percent_yr if insurance_percent_yr is not None else 0
    )
    ...
    disconnect_cost_per_kwh = (
        disconnect_cost_per_kwh if disconnect_cost_per_kwh is not None else 0
    )
    recycling_cost_per_kwh = (
        recycling_cost_per_kwh if recycling_cost_per_kwh is not None else 0
    )
    ...
    if net_initial_cost < 0:
        print("Warning: Total incentives exceed calculated initial BESS cost.")
    ...
    if om_cost_per_kwh_yr is not None:
        ...
        fixed_om_per_kw_yr is not None
    ):  # Check fixed_om_per_kw_yr which now has a default
    ...
    insurance_cost = (
        insurance_percent_yr / 100.0
    ) * total_initial_cost  # Insurance based on gross cost
    ...
    if battery_replacement_interval <= 0 or pd.isna(battery_replacement_interval):
        ...
        print(
            "Warning: Calculated battery replacement interval is zero or invalid. No replacements will be scheduled."
        )
    ...
    for year in range(1, years + 1):
        ...
        if battery_replacement_interval != float("inf"):
            if (
                year > 0
                and abs(year % battery_replacement_interval) < 0.01
                or abs(
                    year % battery_replacement_interval - battery_replacement_interval
                )
                < 0.01
            ):
                inflated_replacement_cost = total_initial_cost * (
                    (1 + inflation_rate) ** (year - 1)
                )
                replacement_cost_year = inflated_replacement_cost
                print(
                    f"DEBUG: Battery replacement cost ${replacement_cost_year:,.0f} applied in year {year} (Interval: {battery_replacement_interval:.1f} yrs)"
                )
                ...
                replacement_cost_year = (
                    inflated_replacement_cost * replacements_per_year
                )
                print(
                    f"DEBUG: Multiple ({replacements_per_year}) battery replacements costing ${replacement_cost_year:,.0f} applied in year {year}"
                )
        ...
        if year == years:
            ...
            inflated_decomm_cost = decomm_cost_base * (
                (1 + inflation_rate) ** (year - 1)
            )
    ...
    if cash_flows[0] >= 0:
        ...
        for year_pbk in range(1, len(cash_flows)):
            ...
            if current_year_cf is None:
                current_year_cf = 0  # Handle potential None in cash flows
            if cumulative_cash_flow + current_year_cf >= 0:
                fraction_needed = (
                    abs(cumulative_cash_flow) / current_year_cf
                    if current_year_cf is not None and current_year_cf > 0
                    else 0
                )
...
def safe_calculate_annual_billings(eaf_params, bess_params, utility_params):
    """Safe wrapper for calculate_annual_billings that handles exceptions."""
    try:
        return calculate_annual_billings(eaf_params, bess_params, utility_params)
    except Exception as e:
        print(f"Error in annual billings calculation (hidden from user): {str(e)}")
        # Return a default structure with zeros
        return {
            "monthly_bills_with_bess": [{"total_bill": 0, "peak_demand_kw": 0} for _ in range(12)],
            "monthly_bills_without_bess": [{"total_bill": 0, "peak_demand_kw": 0} for _ in range(12)],
            "monthly_savings": [0] * 12,
            "annual_bill_with_bess": 0,
            "annual_bill_without_bess": 0,
            "annual_savings": 0
        }

def safe_calculate_incentives(bess_params, incentive_params):
    """Safe wrapper for calculate_incentives that handles exceptions."""
    try:
        return calculate_incentives(bess_params, incentive_params)
    except Exception as e:
        print(f"Error in incentives calculation (hidden from user): {str(e)}")
        # Return a default structure
        return {
            "total_incentive": 0,
            "breakdown": {},
            "calculated_initial_cost": 0
        }

def safe_calculate_financial_metrics(bess_params, financial_params, eaf_params, annual_savings, incentives_results):
    """Safe wrapper for calculate_financial_metrics that handles exceptions."""
    try:
        return calculate_financial_metrics(bess_params, financial_params, eaf_params, annual_savings, incentives_results)
    except Exception as e:
        print(f"Error in financial metrics calculation (hidden from user): {str(e)}")
        # Return a default structure
        years = int(financial_params.get(KEY_LIFESPAN, 30)) if financial_params else 30
        return {
            "npv": 0,
            "irr": 0,
            "payback_years": float('inf'),
            "cash_flows": [0] * (years + 1),
            "net_initial_cost": 0,
...
def safe_create_metrics_card(financial_metrics):
    ...
    try:
        ...
        def fmt_p(v): 
            try:
                return f"{v:.1%}" if pd.notna(v) and isinstance(v, (int, float)) and abs(v)<=5 else (f"{'+' if v > 0 else ''}>500%" if pd.notna(v) and abs(v)>5 else "N/A")
            except:
                return "N/A"
                
        def fmt_y(v): 
            try:
                return "Never" if pd.isna(v) or v == float("inf") else ("< 0 (Immediate)" if v < 0 else f"{v:.1f} yrs")
            except:
                return "N/A"
        ...
        print(f"Error creating metrics card (hidden from user): {str(e)}")
        # Return a simple fallback card
        return dbc.Card([
            dbc.CardHeader("Financial Summary"),
            dbc.CardBody("Adjust parameters to see financial metrics")
        ])

def safe_create_savings_card(billing_results, financial_metrics):
    """Safely creates the savings card."""
    try:
        def fmt_c(v): 
            try:
                return f"${v:,.0f}" if pd.notna(v) else "$0"
            except:
                return "$0"
        ...
        print(f"Error creating savings card (hidden from user): {str(e)}")
        # Return fallback
        return dbc.Card([
            dbc.CardHeader("Annual Costs & Savings"),
            dbc.CardBody("Adjust parameters to see cost analysis")
        ])

def safe_create_incentives_card(incentive_results):
    """Safely creates the incentives card."""
    try:
        def fmt_c(v): 
            try:
                return f"${v:,.0f}" if pd.notna(v) else "$0"
        ...
        return incentives_card
    except Exception as e:
        print(f"Error creating incentives card (hidden from user): {str(e)}")
        # Return fallback
        return dbc.Card([
            dbc.CardHeader("Incentives Applied"),
            dbc.CardBody("Select applicable incentives to see benefits")
        ])

def safe_create_monthly_table(billing_results):
    """Safely creates the monthly billing table."""
    try:
        months = [calendar.month_abbr[i] for i in range(1, 13)]
        
        # Safely get values or use zeros
        bills_without = billing_results.get("monthly_bills_without_bess", [{"total_bill": 0, "peak_demand_kw": 0} for _ in range(12)])
        bills_with = billing_results.get("monthly_bills_with_bess", [{"total_bill": 0, "peak_demand_kw": 0} for _ in range(12)])
        savings = billing_results.get("monthly_savings", [0] * 12)
        
        # Handle possible length mismatches
        while len(bills_without) < 12:
            bills_without.append({"total_bill": 0, "peak_demand_kw": 0})
        while len(bills_with) < 12:
            bills_with.append({"total_bill": 0, "peak_demand_kw": 0})
        while len(savings) < 12:
            savings.append(0)
            
        # Create DataFrame with error checking
        df_monthly = pd.DataFrame({
            "Month": months,
            "Bill Without BESS": [b.get("total_bill", 0) for b in bills_without[:12]],
            "Bill With BESS": [b.get("total_bill", 0) for b in bills_with[:12]],
            "Savings": savings[:12],
            "Peak Demand w/o BESS (kW)": [b.get("peak_demand_kw", 0) for b in bills_without[:12]],
            "Peak Demand w/ BESS (kW)": [b.get("peak_demand_kw", 0) for b in bills_with[:12]],
        })
        ...
        for col in ["Bill Without BESS", "Bill With BESS", "Savings"]:
            df_monthly[col] = df_monthly[col].apply(fmt_c)
        for col in ["Peak Demand w/o BESS (kW)", "Peak Demand w/ BESS (kW)"]:
            df_monthly[col] = df_monthly[col].apply(fmt_kw)
        ...
        print(f"Error creating monthly table (hidden from user): {str(e)}")
        # Return fallback
        return html.Div("Monthly billing data will appear here")
...
def display_calculation_results(n_clicks, eaf_params, bess_params, utility_params, financial_params, incentive_params, validation_errors):
    ...
    # Always ensure error display is off
    error_output = "" 
    error_open = False
    ...
    # Ensure all parameters exist and have valid default values if missing
    eaf_params = sanitize_eaf_params(eaf_params or nucor_mills["Custom"].copy())
    bess_params = sanitize_bess_params(bess_params or default_bess_params_store.copy())
    utility_params = sanitize_utility_params(utility_params or utility_rates["Custom Utility"].copy())
    financial_params = sanitize_financial_params(financial_params or default_financial_params.copy())
    incentive_params = incentive_params or default_incentive_params.copy()
    
    try:
        # --- Perform Calculations ---
        billing_results = safe_calculate_annual_billings(eaf_params, bess_params, utility_params)
        incentive_results = safe_calculate_incentives(bess_params, incentive_params)
        financial_metrics = safe_calculate_financial_metrics(bess_params, financial_params, eaf_params, 
                                                           billing_results["annual_savings"], incentive_results)

        # Store results
        stored_data = {"billing": billing_results, "incentives": incentive_results, "financials": financial_metrics, 
                      "inputs": {"eaf": eaf_params, "bess": bess_params, "utility": utility_params, 
                                "financial": financial_params, "incentive": incentive_params}}

        # --- Generate Plot Data Safely ---
        time_plot, eaf_power_plot, grid_power_plot, bess_power_plot, max_y_plot = safe_generate_plot_data(eaf_params, bess_params)

        # --- Build UI Components Safely ---
        metrics_card = safe_create_metrics_card(financial_metrics)
        savings_card = safe_create_savings_card(billing_results, financial_metrics)
        incentives_card = safe_create_incentives_card(incentive_results)
        monthly_table = safe_create_monthly_table(billing_results)
        tech_comparison = safe_create_tech_comparison(bess_params.get(KEY_TECH, "LFP"), 
                                                    bess_params.get(KEY_CAPACITY, 0), 
                                                    bess_params.get(KEY_POWER_MAX, 0))
        
        # Graphs - wrapped in try/except blocks
        fig_cashflow = safe_create_cashflow_graph(financial_metrics, financial_params)
        fig_cumulative_cashflow = safe_create_cumulative_cashflow_graph(financial_metrics, financial_params)
        fig_single_cycle = safe_create_cycle_graph(time_plot, eaf_power_plot, grid_power_plot, bess_power_plot, 
                                                  max_y_plot, eaf_params)

        # --- Assemble Results Output (Without Errors) ---
        ...
        print(f"Calculation error (hidden from user): {str(e)}")
        # Instead of showing an error, show a graceful fallback message
        results_output = html.Div([
            html.H3("Calculation Results", className="mb-4"),
            html.P("Please adjust your input parameters and try again.", className="text-center"),
            html.P("For best results, ensure all inputs have reasonable values.", className="text-center text-muted"),
        ])
        
    # Always ensure error display is off
    return results_output, stored_data, "", False

# Validation callback update - disable error display
@app.callback(
    ...
    [Input(ID_CALC_BTN, "n_clicks"), Input(ID_OPTIMIZE_BTN, "n_clicks"), 
     Input(STORE_UTILITY, "data"), Input(STORE_EAF, "data"), Input(STORE_BESS, "data"), 
     Input(STORE_FINANCIAL, "data")],
    prevent_initial_call=True,
)
def validate_inputs(calc_clicks, opt_clicks, utility_params, eaf_params, bess_params, fin_params):
    # Instead of showing errors, just return empty/false
    return "", False, False
...
def display_optimization_results(n_clicks, eaf_params, bess_base_params, utility_params, financial_params, incentive_params, validation_errors):
    ...
    # Sanitize inputs regardless of validation errors
    eaf_params = sanitize_eaf_params(eaf_params or nucor_mills["Custom"].copy())
    bess_base_params = sanitize_bess_params(bess_base_params or default_bess_params_store.copy())
    utility_params = sanitize_utility_params(utility_params or utility_rates["Custom Utility"].copy())
    financial_params = sanitize_financial_params(financial_params or default_financial_params.copy())
    incentive_params = incentive_params or default_incentive_params.copy()
    
    try:
        # Safely optimize battery size
        technology = bess_base_params.get(KEY_TECH, "LFP")
        
        # Define search space (adjust ranges and step count as needed)
        capacity_options = np.linspace(5, 100, 5)  # Reduced steps for faster optimization
        power_options = np.linspace(2, 50, 5)     # Reduced steps for faster optimization
        ...
        for capacity in capacity_options:
            for power in power_options:
                ...
                try:
                    # Run calculations for this specific size (using safe wrappers)
                    billing_results = safe_calculate_annual_billings(eaf_params, test_bess_params, utility_params)
                    annual_savings = billing_results["annual_savings"]
                    incentive_results = safe_calculate_incentives(test_bess_params, incentive_params)
                    metrics = safe_calculate_financial_metrics(test_bess_params, financial_params, eaf_params, 
                                                             annual_savings, incentive_results)
        ...
        if best_capacity is not None and best_power is not None:
            # Format functions
            ...
            best_summary = dbc.Card([
                dbc.CardHeader(f"Optimal Size Found (Max NPV) - Tech: {technology}"),
                dbc.CardBody(html.Table([
                    html.Tr([html.Td("Capacity (MWh)"), html.Td(f"{best_capacity:.1f}")]),
                    html.Tr([html.Td("Power (MW)"), html.Td(f"{best_power:.1f}")]),
                    html.Tr([html.Td("Resulting NPV"), html.Td(fmt_c(best_npv))]),
            ...
            all_results_df = pd.DataFrame(optimization_results)
            if not all_results_df.empty:
                ...
                available_cols = [k for k in display_cols if k in all_results_df.columns]
                
                if available_cols:
                    all_results_df = all_results_df[available_cols].copy()
                    all_results_df.rename(columns={k: display_cols[k] for k in available_cols}, inplace=True)
                    
                    # Format columns safely
                    for col in ["Capacity (MWh)", "Power (MW)"]:
                        if col in all_results_df.columns:
                            all_results_df[col] = all_results_df[col].apply(lambda x: f"{x:.1f}" if pd.notna(x) else "N/A")
                            
                    for col in ["NPV ($)", "Savings ($/Yr)", "Net Cost ($)"]:
                        if col in all_results_df.columns:
                            all_results_df[col] = all_results_df[col].apply(fmt_c)
                            
                    if "IRR (%)" in all_results_df.columns:
                        all_results_df["IRR (%)"] = all_results_df["IRR (%)"].apply(fmt_p)
                        
                    if "Payback (Yrs)" in all_results_df.columns:
                    ...
                    table_section = html.Div("No detailed optimization results available.")
            else:
            ...
            opt_output = html.Div([
                html.H3("Battery Sizing Optimization", className="mb-4"),
                html.P("No optimal battery size could be determined with the current parameters.", className="text-center"),
                html.P("Try adjusting your input parameters and run the optimization again.", className="text-center text-muted")
            ])

    except Exception as e:
        # Log error but don't display to user
        print(f"Optimization error (hidden from user): {str(e)}")
        # Show friendly message
        opt_output = html.Div([
            html.H3("Battery Sizing Optimization", className="mb-4"),
            html.P("Please adjust your parameters and try again.", className="text-center"),
            html.P("For best results, ensure all inputs have reasonable values.", className="text-center text-muted")
        ])
...
def create_technology_comparison_table(current_tech, capacity_mwh, power_mw):
    ...
    # Fix: Add null checking for capacity_mwh and power_mw
                                                ...
                                                dbc.Card(
                                                    ...
                                                    children=[  # BESS Card
                                                        ...
                                                        html.Div(
                                                            ...
                                                            className="mt-2 mb-2 text-muted small",
                                                        ),  # C-Rate Display
                                                        html.Hr(),
                                                                ...
                                                                dbc.CardBody(
                                                                    [
                                                                        create_bess_input_group(
                                                                            ...
                                                                            "$/kWh",
                                                                            tooltip_text="Storage Block (Cells, Modules, Racks) + Balance of System (HVAC, Fire Suppression, etc.) cost per kWh.",
                                                                        ),
                                                                        create_bess_input_group(
                                                                            "PCS Cost:",
                                                                            ID_BESS_PCS_COST,
                                                                            ...
                                                                            "$/kW",
                                                                            tooltip_text="Power Conversion System (Inverters, Transformers) cost per kW.",
                                                                        ),
                                                                        create_bess_input_group(
                                                                            "System Integration:",
                                                                        ...
                                                                        create_bess_input_group(
                                                                            ...
                                                                            "$/kWh",
                                                                            tooltip_text="Engineering, Procurement, Construction (Installation Labor, Site Prep) cost per kWh.",
                                                                        ),
                                                                    ]
                                                                ),
                                                            ],
                                                                ...
                                                                dbc.CardBody(
                                                                    [
                                                                        create_bess_input_group(
                                                                            ...
                                                                            "$/kWh",
                                                                            min_val=None,
                                                                            tooltip_text="Cost to disconnect and remove the system at end-of-life.",
                                                                        ),  # Allow negative later?
                                                                        create_bess_input_group(
                                                                            "Recycling/Disposal:",
                                                                            ...
                                                                            "$/kWh",
                                                                            min_val=None,
                                                                            tooltip_text="Net cost of recycling or disposing of components (can be negative if valuable materials).",
                                                                        ),  # Allow negative
                                                                    ]
                                                                ),
                                                            ],
                                                                ...
                                                                dbc.CardBody(
                                                                    [
                                                                        create_bess_input_group(
                                                                            ...
                                                                            "cycles",
                                                                            min_val=100,
                                                                            tooltip_text="Number of full charge/discharge cycles until end-of-life (e.g., 80% capacity retention).",
                                                                        ),
                                                                        create_bess_input_group(
                                                                            "Depth of Discharge:",
                                                                            ID_BESS_DOD,
                                                                            ...
                                                                            "%",
                                                                            min_val=1,
                                                                            tooltip_text="Recommended maximum percentage of capacity to discharge per cycle.",
                                                                        ),
                                                                        create_bess_input_group(
                                                                            "Calendar Life:",
                                                                            ...
                                                                            "years",
                                                                            min_val=1,
                                                                            tooltip_text="Expected lifespan based on time, regardless of cycles.",
                                                                        ),
                                                                    ]
                                                                ),
                                                            ]
                                                    ...
                                                    children=[  # Financial Card
                                                        ...
                                                        html.Div(
                                                            [
                                                                ...
                                                                html.P(
                                                                    "Note: End-of-life value primarily driven by Decommissioning costs.",
                                                                    className="small text-muted",
                                                                ),
                                                            ],
                                                            className="mb-2",
                        ...
                        dbc.Container(
                            children=[
                                dcc.Loading(
                                    ...
                                    type="circle",
                                    children=[
                                        html.Div(id=ID_RESULTS_OUTPUT, className="mt-4")
                                    ],
                                )
                            ]
                        )
...
def navigate_tabs(n_params, n_incentives):
    ...
    if ctx.triggered_id == ID_CONTINUE_PARAMS_BTN:
        return "tab-params"
    elif ctx.triggered_id == ID_CONTINUE_INCENTIVES_BTN:
...
def update_eaf_params_store(
    ...
    if triggered_id == ID_MILL_DROPDOWN:
        ...
        if count is not None:
            output_data["eaf_count"] = count  # Assuming key is "eaf_count"
    ...
    try:
        base_duration = float(base_duration) if base_duration is not None else 0.0
    except (ValueError, TypeError):
        base_duration = 0.0
    try:
        input_duration = float(input_duration) if input_duration is not None else 0.0
    except (ValueError, TypeError):
        input_duration = 0.0
    ...
    if input_duration > 0:
        ...
        output_data[KEY_CYCLE_DURATION] = (
            base_duration  # Use mill default if input invalid
        )
...
def update_financial_params_store(wacc, lifespan, tax, inflation, salvage):
    ...
    return {
        KEY_WACC: (
            wacc / 100.0 if wacc is not None else default_financial_params[KEY_WACC]
        ),
        KEY_LIFESPAN: lifespan,
        KEY_TAX_RATE: (
            tax / 100.0 if tax is not None else default_financial_params[KEY_TAX_RATE]
        ),
        KEY_INFLATION: (
            inflation / 100.0
            if inflation is not None
            else default_financial_params[KEY_INFLATION]
        ),
        KEY_SALVAGE: (
            salvage / 100.0
            if salvage is not None
            else default_financial_params[KEY_SALVAGE]
        ),
    }
...
def update_utility_params_store(
    ...
    winter_mult,
    summer_mult,
    shoulder_mult,  # Values from inputs
    winter_months_str,
    summer_months_str,
    shoulder_months_str,  # Values from inputs
    tou_starts,
    tou_ends,
    tou_rates,
    existing_data,
    ...
    def parse_months(month_str, default_list):
        ...
        try:
            ...
            print(
                f"Warning: Could not parse month string '{month_str}'. Using default."
            )
            return default_list
    ...
    if is_seasonal_enabled:
        print(
            "DEBUG Utility Store: Seasonal Toggle is ON. Updating seasonal params from inputs."
        )
        ...
        params[KEY_WINTER_MULT] = (
            winter_mult
            if winter_mult is not None
            else default_utility_params[KEY_WINTER_MULT]
        )
        params[KEY_SUMMER_MULT] = (
            summer_mult
            if summer_mult is not None
            else default_utility_params[KEY_SUMMER_MULT]
        )
        params[KEY_SHOULDER_MULT] = (
            shoulder_mult
            if shoulder_mult is not None
            else default_utility_params[KEY_SHOULDER_MULT]
        )
        # TODO: Add validation here to ensure all 12 months are covered and unique
    else:
        print(
            "DEBUG Utility Store: Seasonal Toggle is OFF. Using default/existing seasonal params (resetting to defaults)."
        )
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            html.Span(
                                f"{mill_data.get('eaf_count', 'N/A')} x {mill_data.get(KEY_EAF_SIZE, 'N/A')} ton {mill_data.get('eaf_type', 'N/A')} ({mill_data.get('eaf_manufacturer', 'N/A')})"
                            ),
                        ],
                        className="mb-1",
                    ),
                    ...
                    html.Div(
                        [
                            ...
                            html.Span(
                                f"{mill_data.get(KEY_CYCLES_PER_DAY, 'N/A')} cycles/day, {mill_data.get(KEY_DAYS_PER_YEAR, 'N/A')} days/year, {mill_data.get(KEY_CYCLE_DURATION, 'N/A')} min/cycle"
                            ),
                        ],
                        className="mb-1",
                    ),
                    ...
                    html.Div(
                        [
                            ...
                            html.Span(f"{mill_data.get(KEY_GRID_CAP, 'N/A')} MW"),
                        ],
                        className="mb-1",
                    ),
                ]
...
@app.callback(
    [
        Output(ID_UTILITY_DROPDOWN, "value"),
        Output(ID_OFF_PEAK, "value"),
        Output(ID_MID_PEAK, "value"),
        Output(ID_PEAK, "value"),
        Output(ID_DEMAND_CHARGE, "value"),
        Output(ID_SEASONAL_TOGGLE, "value"),
        Output(ID_EAF_SIZE, "value"),
        Output(ID_EAF_COUNT, "value"),
        Output(ID_GRID_CAP, "value"),
        Output(ID_CYCLES_PER_DAY, "value"),
        Output(ID_CYCLE_DURATION, "value"),
        Output(ID_DAYS_PER_YEAR, "value"),
...
def modify_tou_rows(add_clicks, remove_clicks_list, current_rows):
    ...
    if triggered_input == ID_ADD_TOU_BTN:
        ...
        if new_rows:
            try:
                last_row_end_input = current_rows[-1]["props"]["children"][0]["props"][
                    "children"
                ][1]["props"][
                    "children"
                ]  # Navigate to end input
...
@app.callback(
    ...
    [
        Input(ID_CALC_BTN, "n_clicks"),
        Input(ID_OPTIMIZE_BTN, "n_clicks"),
        Input(STORE_UTILITY, "data"),
        Input(STORE_EAF, "data"),
        Input(STORE_BESS, "data"),
        Input(STORE_FINANCIAL, "data"),
    ],
    prevent_initial_call=True,
)
def validate_inputs(
    ...
    if not utility_params:
        ...
        if rates.get("off_peak", -1) < 0:
            errors.append("Off-Peak Rate cannot be negative.")
        if rates.get("mid_peak", -1) < 0:
            errors.append("Mid-Peak Rate cannot be negative.")
        if rates.get("peak", -1) < 0:
            errors.append("Peak Rate cannot be negative.")
        if utility_params.get(KEY_DEMAND_CHARGE, -1) < 0:
            errors.append("Demand Charge cannot be negative.")
    ...
    if not eaf_params:
        ...
        if eaf_params.get(KEY_EAF_SIZE, 0) <= 0:
            errors.append("EAF Size must be positive.")
        if eaf_params.get("eaf_count", 0) <= 0:
            errors.append("Number of EAFs must be positive.")
        if eaf_params.get(KEY_GRID_CAP, 0) <= 0:
            errors.append("Grid Power Limit must be positive.")
        if eaf_params.get(KEY_CYCLES_PER_DAY, 0) <= 0:
            errors.append("EAF Cycles per Day must be positive.")
        if eaf_params.get(KEY_CYCLE_DURATION_INPUT, 0) <= 0:
            errors.append("Avg. Cycle Duration must be positive.")
        if not (0 < eaf_params.get(KEY_DAYS_PER_YEAR, 0) <= 366):
            errors.append("Operating Days per Year must be between 1 and 366.")

    # BESS Validation
    if not bess_params:
        errors.append("BESS parameters are missing.")
    else:
        if bess_params.get(KEY_CAPACITY, 0) <= 0:
            errors.append("BESS Capacity (MWh) must be positive.")
        if bess_params.get(KEY_POWER_MAX, 0) <= 0:
            errors.append("BESS Power (MW) must be positive.")
        if bess_params.get(KEY_SB_BOS_COST, -1) < 0:
            errors.append("BESS SB+BOS Cost cannot be negative.")
        if bess_params.get(KEY_PCS_COST, -1) < 0:
            errors.append("BESS PCS Cost cannot be negative.")
        if bess_params.get(KEY_EPC_COST, -1) < 0:
            errors.append("BESS EPC Cost cannot be negative.")
        if bess_params.get(KEY_SYS_INT_COST, -1) < 0:
            errors.append("BESS System Integration Cost cannot be negative.")
        if (
            bess_params.get(KEY_FIXED_OM, None) is not None
            and bess_params.get(KEY_FIXED_OM, -1) < 0
        ):
            errors.append("BESS Fixed O&M Cost cannot be negative.")
        if (
            bess_params.get(KEY_OM_KWHR_YR, None) is not None
            and bess_params.get(KEY_OM_KWHR_YR, -1) < 0
        ):
            errors.append("BESS O&M Cost ($/kWh/yr) cannot be negative.")
        if not (0 < bess_params.get(KEY_RTE, 0) <= 100):
            errors.append("BESS RTE must be between 0% and 100%.")
        if bess_params.get(KEY_INSURANCE, -1) < 0:
            errors.append("BESS Insurance Rate cannot be negative.")
        # Decomm costs can be negative
        if bess_params.get(KEY_CYCLE_LIFE, 0) <= 0:
            errors.append("BESS Cycle Life must be positive.")
        if not (0 < bess_params.get(KEY_DOD, 0) <= 100):
            errors.append("BESS DoD must be between 0% and 100%.")
        if bess_params.get(KEY_CALENDAR_LIFE, 0) <= 0:
            errors.append("BESS Calendar Life must be positive.")

    # Financial Validation
    if not fin_params:
        errors.append("Financial parameters are missing.")
    else:
        if not (0 <= fin_params.get(KEY_WACC, -1) <= 1):
            errors.append("WACC must be between 0% and 100%.")
        if fin_params.get(KEY_LIFESPAN, 0) <= 0:
            errors.append("Project Lifespan must be positive.")
        if not (0 <= fin_params.get(KEY_TAX_RATE, -1) <= 1):
            errors.append("Tax Rate must be between 0% and 100%.")
        # Allow deflation, but maybe warn if too extreme?
        if fin_params.get(KEY_INFLATION, None) is None:
            errors.append("Inflation Rate is missing.")
        if not (0 <= fin_params.get(KEY_SALVAGE, -1) <= 1):
    ...
    if errors:
        ...
        output_elements.append(
            html.H5("Validation Warnings:", className="text-warning")
        )
...
def sanitize_bess_params(bess_params):
    """Ensures all BESS parameters have valid numeric values."""
    if not bess_params:
        return default_bess_params_store.copy()
    
    safe_params = bess_params.copy()
    
    # Ensure essential parameters exist with valid defaults
    numeric_keys = [KEY_CAPACITY, KEY_POWER_MAX, KEY_SB_BOS_COST, KEY_PCS_COST, 
                   KEY_EPC_COST, KEY_SYS_INT_COST, KEY_FIXED_OM, KEY_OM_KWHR_YR,
                   KEY_RTE, KEY_INSURANCE, KEY_DISCONNECT_COST, KEY_RECYCLING_COST,
                   KEY_CYCLE_LIFE, KEY_DOD, KEY_CALENDAR_LIFE]
    
    # Set technology default if missing
    if KEY_TECH not in safe_params or not safe_params[KEY_TECH]:
        safe_params[KEY_TECH] = "LFP"
    ...
    for key in numeric_keys:
        if key in safe_params:
            try:
                ...
                if safe_params[key] is None or pd.isna(safe_params[key]):
                    if key in default_bess_params_store:
                        safe_params[key] = default_bess_params_store[key]
                    else:
                        safe_params[key] = 0
                else:
...
def sanitize_eaf_params(eaf_params):
    """Ensures all EAF parameters have valid numeric values."""
    if not eaf_params:
        return nucor_mills["Custom"].copy()
    
    safe_params = eaf_params.copy()
    
    # Numeric keys to check
    numeric_keys = [KEY_EAF_SIZE, "eaf_count", KEY_GRID_CAP, KEY_CYCLES_PER_DAY, 
                   KEY_CYCLE_DURATION, KEY_CYCLE_DURATION_INPUT, KEY_DAYS_PER_YEAR]
    ...
    for key in numeric_keys:
        if key in safe_params:
            try:
                if safe_params[key] is None or pd.isna(safe_params[key]):
                    if key in nucor_mills["Custom"]:
                        safe_params[key] = nucor_mills["Custom"][key]
                    else:
                        safe_params[key] = 1  # Safe minimum default
                else:
                    safe_params[key] = float(safe_params[key])
            except (ValueError, TypeError):
                if key in nucor_mills["Custom"]:
...
def sanitize_financial_params(financial_params):
    """Ensures all financial parameters have valid numeric values."""
    if not financial_params:
        return default_financial_params.copy()
    
    safe_params = financial_params.copy()
    
    # Numeric keys to check
    numeric_keys = [KEY_WACC, KEY_LIFESPAN, KEY_TAX_RATE, KEY_INFLATION, KEY_SALVAGE]
    ...
    for key in numeric_keys:
        if key in safe_params:
            try:
                if safe_params[key] is None or pd.isna(safe_params[key]):
                    safe_params[key] = default_financial_params[key]
                else:
                    safe_params[key] = float(safe_params[key])
            except (ValueError, TypeError):
                safe_params[key] = default_financial_params[key]
...
def sanitize_utility_params(utility_params):
    """Ensures all utility parameters have valid values."""
    if not utility_params:
        return default_utility_params.copy()
    
    safe_params = utility_params.copy()
    
    # Ensure energy rates exist and are valid
    if KEY_ENERGY_RATES not in safe_params or not safe_params[KEY_ENERGY_RATES]:
        safe_params[KEY_ENERGY_RATES] = default_utility_params[KEY_ENERGY_RATES].copy()
    else:
        # Ensure all rate types exist
        for rate_type in ["off_peak", "mid_peak", "peak"]:
            if rate_type not in safe_params[KEY_ENERGY_RATES] or safe_params[KEY_ENERGY_RATES][rate_type] is None:
                safe_params[KEY_ENERGY_RATES][rate_type] = default_utility_params[KEY_ENERGY_RATES][rate_type]
            try:
                safe_params[KEY_ENERGY_RATES][rate_type] = float(safe_params[KEY_ENERGY_RATES][rate_type])
            except (ValueError, TypeError):
                safe_params[KEY_ENERGY_RATES][rate_type] = default_utility_params[KEY_ENERGY_RATES][rate_type]
    
    # Ensure demand charge is valid
    if KEY_DEMAND_CHARGE not in safe_params or safe_params[KEY_DEMAND_CHARGE] is None:
        safe_params[KEY_DEMAND_CHARGE] = default_utility_params[KEY_DEMAND_CHARGE]
    try:
        safe_params[KEY_DEMAND_CHARGE] = float(safe_params[KEY_DEMAND_CHARGE])
    except (ValueError, TypeError):
        safe_params[KEY_DEMAND_CHARGE] = default_utility_params[KEY_DEMAND_CHARGE]
    
    # Ensure TOU periods are valid
    if KEY_TOU_FILLED not in safe_params or not safe_params[KEY_TOU_FILLED]:
        if KEY_TOU_RAW in safe_params and safe_params[KEY_TOU_RAW]:
            safe_params[KEY_TOU_FILLED] = fill_tou_gaps(safe_params[KEY_TOU_RAW])
        else:
            safe_params[KEY_TOU_FILLED] = default_utility_params[KEY_TOU_FILLED].copy()
...
def update_bess_inputs_from_technology(selected_technology):
    ...
    if not selected_technology or selected_technology not in bess_technology_data:
        print(
            f"DEBUG: Invalid or no technology selected ('{selected_technology}'), defaulting to LFP"
        )
    ...
    style_fixed_om = (
        {"display": "none"} if show_om_kwhyr else {"display": "flex"}
    )  # Use flex for Row display
    style_om_kwhyr = (
        {"display": "flex"} if show_om_kwhyr else {"display": "none"}
    )  # Use flex for Row display
...
@app.callback(
    [
        ...
        Output(ID_CALCULATION_ERR, "is_open"),
    ],
    Input(ID_CALC_BTN, "n_clicks"),
    [
        State(STORE_EAF, "data"),
...
def display_calculation_results(
    ...
    if not all(
        [eaf_params, bess_params, utility_params, financial_params, incentive_params]
    ):
        print(
            "ERROR: One or more parameter sets are missing in display_calculation_results"
        )
        error_output = html.Div(
            [
                html.H5("Internal Error", className="text-danger"),
                html.P(
                    "Could not retrieve all required parameters. Please try reloading or check console."
                ),
            ]
        )
        error_open = True
    ...
    try:
        ...
        if KEY_TECH not in bess_params:
            raise ValueError(
                "BESS parameters dictionary is missing the 'technology' key."
            )
        current_technology = bess_params.get(
            KEY_TECH, "LFP"
        )  # Get current tech for display

        print(
            f"DEBUG: Proceeding with calculations using technology: {current_technology}"
        )
        ...
        try:
            ...
            plot_cycle_duration_min = max(1, plot_cycle_duration_min)  # Ensure positive
            time_plot = np.linspace(0, plot_cycle_duration_min, 200)
            eaf_power_plot = calculate_eaf_profile(
                time_plot, eaf_params.get(KEY_EAF_SIZE, 100), plot_cycle_duration_min
            )
            grid_power_plot, bess_power_plot = calculate_grid_bess_power(
                eaf_power_plot,
                eaf_params.get(KEY_GRID_CAP, 35),
                bess_params.get(KEY_POWER_MAX, 20),
            )
            plot_data_calculated = True
            max_y_plot = (
                max(
                    np.max(eaf_power_plot) if len(eaf_power_plot) > 0 else 0,
                    eaf_params.get(KEY_GRID_CAP, 35),
                )
                * 1.15
            )
            max_y_plot = max(10, max_y_plot)  # Ensure min range
        ...
        def fmt_p(v):
            return (
                f"{v:.1%}"
                if pd.notna(v) and isinstance(v, (int, float)) and abs(v) <= 5
                else (
                    f"{'+' if v > 0 else ''}>500%"
                    if pd.notna(v) and abs(v) > 5
                    else "N/A"
                )
            )
        ...
        def fmt_y(v):
            return (
                ...
                else ("< 0 (Immediate)" if v < 0 else f"{v:.1f} yrs")
            )
                        ...
                        [
                            html.Tr(
                                [
                                    ...
                                    html.Td(fmt_c(financial_metrics.get("npv"))),
                                ]
                            ),
                            html.Tr(
                                [
                                    html.Td("Internal Rate of Return (IRR)"),
                                    html.Td(fmt_p(financial_metrics.get("irr"))),
                                ]
                            ),
                            html.Tr(
                                [
                            ...
                            html.Tr(
                                [
                                    html.Td("Est. Battery Life (Replacement Interval)"),
                                    html.Td(
                                        fmt_y(
                                            financial_metrics.get("battery_life_years")
                                        )
                                    ),
                                ]
                            ),
                            html.Tr(
                                [
                                    html.Td("Net Initial Cost (After Incentives)"),
                                    html.Td(
                            ...
                            html.Tr(
                                [
                                    html.Td("Gross Initial Cost (Before Incentives)"),
                                    html.Td(
                                        fmt_c(
        ...
        if 0 < batt_life < 1:
            replacements_per_year = int(1 / batt_life)
            annual_replacement_cost_display = (
                financial_metrics.get("total_initial_cost", 0) * replacements_per_year
            )
                        ...
                        [
                            ...
                            html.Tr(
                                [
                                    ...
                                    html.Td(
                                        fmt_c(
                                            billing_results.get("annual_bill_with_bess")
                                        )
                                    ),
                                ]
                            ),
                            html.Tr(
                                [
                                    html.Td("Gross Annual Utility Savings"),
                                    html.Td(
                                        html.Strong(
                                            fmt_c(billing_results.get("annual_savings"))
                                        )
                                    ),
                                ]
                            ),
                            ...
                            html.Tr(
                                [
                                    html.Td(
                                        "Annualized Replacement Cost (if life < 1yr)"
                                    ),
                                    html.Td(
                                        html.Strong(
                                            fmt_c(annual_replacement_cost_display),
                                            className="text-danger",
                                        )
                                    ),
                                ]
                            ...
                            html.Tr(
                                [
                                    html.Td(
                                        "Net Annual Benefit (Savings - O&M - Repl.)"
                                    ),
                                    html.Td(
                                        html.Strong(
                                            fmt_c(
                                                billing_results.get("annual_savings", 0)
                                                - financial_metrics.get(
                                                    "initial_om_cost_year1", 0
                                                )
                                                - annual_replacement_cost_display
        ...
        inc_items = [
            html.Tr([html.Td(desc), html.Td(fmt_c(amount))])
            for desc, amount in incentive_results.get("breakdown", {}).items()
        ]
        inc_items.append(
            html.Tr(
                [
                    ...
                    html.Td(
                        html.Strong(fmt_c(incentive_results.get("total_incentive")))
                    ),
                ]
            )
        )
        ...
        df_monthly = pd.DataFrame(
            {
                ...
                "Bill Without BESS": [
                    b.get("total_bill", 0)
                    for b in billing_results.get("monthly_bills_without_bess", [])
                ],
                "Bill With BESS": [
                    b.get("total_bill", 0)
                    for b in billing_results.get("monthly_bills_with_bess", [])
                ],
                "Savings": billing_results.get("monthly_savings", []),
                "Peak Demand w/o BESS (kW)": [
                    b.get("peak_demand_kw", 0)
                    for b in billing_results.get("monthly_bills_without_bess", [])
                ],
        ...
        if len(cash_flows_data) != len(years_cf):
            print(
                f"Warning: Cash flow data length ({len(cash_flows_data)}) doesn't match project lifespan + 1 ({len(years_cf)})."
            )
            cash_flows_data = cash_flows_data[: len(years_cf)] + [0] * (
                len(years_cf) - len(cash_flows_data)
            )
        fig_cashflow = go.Figure(
            go.Bar(
                ...
                marker_color=["red" if cf < 0 else "green" for cf in cash_flows_data],
            )
        )
        ...
        fig_cumulative_cashflow = go.Figure(
            go.Scatter(
                x=years_cf,
        ...
        fig_cumulative_cashflow.add_hline(
            y=0, line_width=1, line_dash="dash", line_color="black"
        )
        fig_cumulative_cashflow.update_layout(
        ...
        if plot_data_calculated:
            fig_single_cycle.add_trace(
                go.Scatter(
                    x=time_plot,
                    y=eaf_power_plot,
                    mode="lines",
                    name="EAF Power Demand",
            ...
            fig_single_cycle.add_shape(
                ...
                x0=0,
                y0=grid_cap_val,
                x1=plot_cycle_duration_min,
                y1=grid_cap_val,
                line=dict(color="black", width=2, dash="dash"),
                name="Grid Cap",
            )
            fig_single_cycle.add_annotation(
                x=plot_cycle_duration_min * 0.9,
                y=grid_cap_val + max_y_plot * 0.03,
                text=f"Grid Cap ({grid_cap_val} MW)",
                showarrow=False,
            ...
            fig_single_cycle.update_layout(
                ...
                annotations=[
                    {
                        "text": "Error generating plot data",
                        "xref": "paper",
                        "yref": "paper",
                        "showarrow": False,
                        "font": {"size": 16},
        ...
        results_output = html.Div(
            [
                ...
                html.H4(
                    "Technology Comparison (at current size)", className="mt-4 mb-3"
                ),
                tech_comparison,
                html.H4("Monthly Billing Breakdown", className="mt-4 mb-3"),
                monthly_table,
                ...
                dbc.Row(
                    [
                        dbc.Col(dcc.Graph(figure=fig_cashflow), md=6),
                        dbc.Col(
                            dcc.Graph(figure=fig_cumulative_cashflow), md=6
                        ),  # Add cumulative plot
                    ]
                ),
            ]
        ...
        error_output = html.Div(
            [
                ...
                html.Details(
                    [  # Show parameters used during error
...
def display_optimization_results(
    ...
    if not all(
        ...
        opt_output = dbc.Alert(
            "Cannot Optimize - Missing one or more parameter sets.", color="danger"
        )
        return opt_output, opt_stored_data
    ...
    try:
        ...
        if opt_results and opt_results.get("best_capacity") is not None:
            ...
            best_summary = dbc.Card(
                [
                    dbc.CardHeader(
                        f"Optimal Size Found (Max NPV) - Tech: {bess_base_params.get(KEY_TECH, 'N/A')}"
                    ),
                    dbc.CardBody(
                        html.Table(
                            [
                                ...
                                html.Tr(
                                    [
                                        ...
                                        html.Td(fmt_c(opt_results["best_npv"])),
                                    ]
                                ),
                                html.Tr(
                                    [
                                        html.Td("Resulting IRR"),
                                        html.Td(fmt_p(best_metrics.get("irr"))),
                                    ]
                                ),
                                html.Tr(
                                    [
                                ...
                                html.Tr(
                                    [
                                        ...
                                        html.Td(
                                            fmt_c(best_metrics.get("net_initial_cost"))
                                        ),
                                    ]
                                ),
                            ],
            ...
            if not all_results_df.empty:
                ...
                all_results_df = all_results_df[
                    [k for k in display_cols if k in all_results_df.columns]
                ].copy()  # Handle missing cols
                all_results_df.rename(columns=display_cols, inplace=True)
                # Format columns
                for col in ["Capacity (MWh)", "Power (MW)"]:
                    all_results_df[col] = all_results_df[col].map("{:.1f}".format)
                for col in ["NPV ($)", "Savings ($/Yr)", "Net Cost ($)"]:
                    all_results_df[col] = all_results_df[col].apply(fmt_c)
                all_results_df["IRR (%)"] = all_results_df["IRR (%)"].apply(fmt_p)
                ...
                [
                    ...
                    html.Ul(
                        [
                            ...
                            html.Li(
                                "Errors occurred during the simulation for all tested sizes (check console logs)."
                            ),
                            html.Li(
                                "Parameter ranges for optimization might need adjustment."
                            ),
                            html.Li(
                                "The selected technology might not be profitable under current conditions."
                            ),
                        ]
                    ),
                ],
