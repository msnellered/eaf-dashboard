# Okay, let's address the `ReferenceError` you're seeing in the console. This error typically occurs when a Dash callback tries to reference a component ID in its `Input` or `State` that isn't present in the initial layout structure when the app starts.

# Looking at the error messages and your code:

# 1.  **`winter-multiplier` ReferenceError:** This ID is used as an `Input` in the `update_utility_params_store` callback. However, the `ID_SEASONAL_CONTAINER` which holds `winter-multiplier` (and other seasonal inputs) is initially set to `style={'display': 'none'}` in the layout. While Dash *can* sometimes handle elements hidden with `display: 'none'`, using them directly as `Input` in callbacks that might fire on initial load or when the container is hidden can cause this error.
# 2.  **`bess-fixed-om` ReferenceError:** This ID (and likely `ID_BESS_OM_KWHR_YR`) are used as `Input` in the `update_bess_params_store` callback. These inputs are conditionally hidden within the `ID_BESS_OPEX_CONTAINER` by the `update_bess_inputs_from_technology` callback using `style={'display': 'none'}`. Similar to the seasonal inputs, using them as `Input` when they are hidden can cause issues.

# **The Solution:**

# The most robust way to handle conditionally displayed inputs in callbacks is to:

# 1.  Ensure the components exist in the initial layout structure (don't hide their parent container with `display: 'none'` initially).
# 2.  Use `State` instead of `Input` for the conditionally displayed components in callbacks where their visibility might change *before* the callback fires. The callback will then read their *current* value when triggered by other, always-visible inputs.

# Let's apply this:

# 1.  **Make `ID_SEASONAL_CONTAINER` initially visible:** Remove the `style={'display': 'none'}` from its definition in the layout.
# 2.  **Add a callback to control `ID_SEASONAL_CONTAINER` visibility:** Create a new callback that listens to the `ID_SEASONAL_TOGGLE` and sets the `display` style dynamically *after* the app loads.
# 3.  **Change seasonal inputs to `State`:** In the `update_utility_params_store` callback, change the seasonal inputs (`winter-multiplier`, etc.) from `Input` to `State`. The callback is already triggered by the utility dropdown and the main rate inputs, which is sufficient.
# 4.  **Change BESS Opex inputs to `State`:** In the `update_bess_params_store` callback, change `ID_BESS_FIXED_OM` and `ID_BESS_OM_KWHR_YR` from `Input` to `State`. This callback is triggered by the technology dropdown and other BESS inputs, which is sufficient.

# **Modified Code:**

# ```python
# --- START OF FILE eaf_bess_dashboardv15.py ---
...
nucor_mills = { "West Virginia": {"location": "Apple Grove, WV", "type": "Sheet", "eaf_count": 2, "eaf_type": "DC", "eaf_manufacturer": "SMS",
KEY_EAF_SIZE: 190, KEY_CYCLES_PER_DAY: 26, "tons_per_year": 3000000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36,"utility": "Appalachian Power", KEY_GRID_CAP: 50}, 

"Auburn": {"location": "Auburn, NY", "type": "Bar", "eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "Unknown", KEY_EAF_SIZE: 60, KEY_CYCLES_PER_DAY: 28, "tons_per_year": 510000,
KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36,"utility": "New York State Electric & Gas", KEY_GRID_CAP: 25}, 

"Birmingham": {"location": "Birmingham, AL", "type": "Bar", "eaf_count": 1, "eaf_type": "AC", 
"eaf_manufacturer": "Unknown", KEY_EAF_SIZE: 52, KEY_CYCLES_PER_DAY: 20, "tons_per_year": 310000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Alabama Power",
KEY_GRID_CAP: 20}, 

"Arkansas": {"location": "Blytheville, AR", "type": "Sheet", "eaf_count": 2, "eaf_type": "AC", "eaf_manufacturer": "Demag", KEY_EAF_SIZE: 150, 
KEY_CYCLES_PER_DAY: 28, "tons_per_year": 2500000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 38, "utility": "Entergy Arkansas", 
KEY_GRID_CAP: 45}, 

"Kankakee": {"location": "Bourbonnais, IL", "type": "Bar", "eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "Danieli", 
KEY_EAF_SIZE: 73, KEY_CYCLES_PER_DAY: 39, "tons_per_year": 850000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36,
"utility": "ComEd", KEY_GRID_CAP: 30}, 

"Brandenburg": {"location": "Brandenburg, KY", "type": "Sheet", "eaf_count": 1, 
"eaf_type": "DC", "eaf_manufacturer": "Danieli", KEY_EAF_SIZE: 150, KEY_CYCLES_PER_DAY: 27, "tons_per_year": 1200000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, 
"utility": "LG&E KU", KEY_GRID_CAP: 45}, 

"Hertford": {"location": "Cofield, NC", "type": "Plate", "eaf_count": 1, "eaf_type": "DC", "eaf_manufacturer": "MAN GHH", 
KEY_EAF_SIZE: 150, KEY_CYCLES_PER_DAY: 30, "tons_per_year": 1350000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Dominion Energy", KEY_GRID_CAP: 45}, 

"Crawfordsville": {"location": "Crawfordsville, IN", "type": "Sheet", "eaf_count": 2, "eaf_type": "AC", "eaf_manufacturer": "Brown-Boveri", KEY_EAF_SIZE: 118, KEY_CYCLES_PER_DAY: 27, 
"tons_per_year": 1890000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Duke Energy", KEY_GRID_CAP: 40}, 

"Darlington": {"location": "Darlington, SC", "type": "Bar", 
"eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "MAN GHH", KEY_EAF_SIZE: 110, KEY_CYCLES_PER_DAY: 30, "tons_per_year": 980000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, 
"utility": "Duke Energy", KEY_GRID_CAP: 40}, 

"Decatur": {"location": "Decatur, AL", "type": "Sheet", "eaf_count": 2, "eaf_type": "DC", "eaf_manufacturer": "NKK-SE", KEY_EAF_SIZE: 165, 
KEY_CYCLES_PER_DAY: 20, "tons_per_year": 2000000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Alabama Power", KEY_GRID_CAP: 48}, 

"Gallatin": {"location": "Ghent, KY", 
"type": "Sheet", "eaf_count": 2, "eaf_type": "?", "eaf_manufacturer": "NKK-SE, Danieli", KEY_EAF_SIZE: 175, KEY_CYCLES_PER_DAY: 27, "tons_per_year": 2800000, 
KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Kentucky Utilities", KEY_GRID_CAP: 53}, 

"Hickman": {"location": "Hickman, AR", "type": "Sheet", "eaf_count": 2, 
"eaf_type": "AC", "eaf_manufacturer": "MAN GHH", KEY_EAF_SIZE: 150, KEY_CYCLES_PER_DAY: 22, "tons_per_year": 2000000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": 
"Entergy Arkansas", KEY_GRID_CAP: 45}, 

"Berkeley": {"location": "Huger, SC", "type": "Sheet/Beam Mill", "eaf_count": 2, "eaf_type": "?", "eaf_manufacturer": "MAN GHH", KEY_EAF_SIZE: 154, 
KEY_CYCLES_PER_DAY: 26, "tons_per_year": 2430000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Santee Cooper", KEY_GRID_CAP: 46}, 

"Texas": {"location": "Jewett, TX", 
"type": "Bar", "eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "SMS Concast", KEY_EAF_SIZE: 100, KEY_CYCLES_PER_DAY: 33, "tons_per_year": 1000000, KEY_DAYS_PER_YEAR: 300, 
KEY_CYCLE_DURATION: 36, "utility": "Oncor Electric Delivery", KEY_GRID_CAP: 30}, 

"Kingman": {"location": "Kingman, AZ", "type": "Bar", "eaf_count": 1, "eaf_type": "AC", 
"eaf_manufacturer": "Danieli", KEY_EAF_SIZE: 100, KEY_CYCLES_PER_DAY: 21, "tons_per_year": 630000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": 
"UniSource Energy Services", KEY_GRID_CAP: 30}, 

"Marion": {"location": "Marion, OH", "type": "Bar Mill/Sign Pos", "eaf_count": 1, "eaf_type": "?", "eaf_manufacturer": "?", 
KEY_EAF_SIZE: 100, KEY_CYCLES_PER_DAY: 40, "tons_per_year": 1200000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "AEP Ohio", KEY_GRID_CAP: 30}, 

"Nebraska": {"location": "Nebraska, NE", "type": "Bar", "eaf_count": 1, "eaf_type": "?", "eaf_manufacturer": "MAN GHH", KEY_EAF_SIZE: 95, KEY_CYCLES_PER_DAY: 35, 
"tons_per_year": 1000000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Nebraska Public Power District", KEY_GRID_CAP: 29}, 

"Utah": {"location": "Plymouth, UT", 
"type": "Bar", "eaf_count": 2, "eaf_type": "?", "eaf_manufacturer": "Fuchs", KEY_EAF_SIZE: 51, KEY_CYCLES_PER_DAY: 42, "tons_per_year": 1290000, KEY_DAYS_PER_YEAR: 300, 
KEY_CYCLE_DURATION: 36, "utility": "Rocky Mountain Power", KEY_GRID_CAP: 15}, 

"Seattle": {"location": "Seattle, WA", "type": "Bar", "eaf_count": 1, "eaf_type": "?", "eaf_manufacturer": 
"Fuchs", KEY_EAF_SIZE: 100, KEY_CYCLES_PER_DAY: 29, "tons_per_year": 855000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Seattle City Light", KEY_GRID_CAP: 30}, 

"Sedalia": {"location": "Sedalia, MO", "type": "Bar", "eaf_count": 1, "eaf_type": "?", "eaf_manufacturer": "Danieli", KEY_EAF_SIZE: 40, KEY_CYCLES_PER_DAY: 39, "tons_per_year": 470000, 
KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Evergy", KEY_GRID_CAP: 12}, 

"Tuscaloosa": {"location": "Tuscaloosa, AL", "type": "Plate", "eaf_count": 1, "eaf_type": "AC", 
"eaf_manufacturer": "MAN GHH", KEY_EAF_SIZE: 122, KEY_CYCLES_PER_DAY: 17, "tons_per_year": 610000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Alabama Power", KEY_GRID_CAP: 37}, 

"Florida": {"location": "Frostproof, FL", "type": "?", "eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "Danieli", KEY_EAF_SIZE: 40, KEY_CYCLES_PER_DAY: 38, "tons_per_year": 450000, 
KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Duke Energy Florida", KEY_GRID_CAP: 12}, 

"Jackson": {"location": "Flowood, MS", "type": "Bar", "eaf_count": 1, "eaf_type": "AC", 
"eaf_manufacturer": "?", KEY_EAF_SIZE: 50, KEY_CYCLES_PER_DAY: 33, "tons_per_year": 490000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Entergy Mississippi (Assumed)", 
KEY_GRID_CAP: 15}, 

"Nucor-Yamato": {"location": "Blytheville, AR", "type": "Structural (implied)", "eaf_count": 2, "eaf_type": "?", "eaf_manufacturer": "?", KEY_EAF_SIZE: 150, 
KEY_CYCLES_PER_DAY: 25, "tons_per_year": 2500000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Mississippi County Electric Cooperative", KEY_GRID_CAP: 45}, 

"Custom": {"location": "Custom Location", "type": "Custom", "eaf_count": 1, "eaf_type": "AC", "eaf_manufacturer": "Custom", KEY_EAF_SIZE: 100, KEY_CYCLES_PER_DAY: 24, 
"tons_per_year": 1000000, KEY_DAYS_PER_YEAR: 300, KEY_CYCLE_DURATION: 36, "utility": "Custom Utility", KEY_GRID_CAP: 35}}
...
def calculate_financial_metrics_advanced(bess_params, financial_params, eaf_params, utility_params, incentive_results):
    ...
    try:
       ...
       # Replace the try/except block in calculate_financial_metrics_advanced (lines 673-682)
        ...
        else: # Fallback if template is missing O&M keys
        ...
        for year in range(1, years + 1):
            ...
            if base_capacity_mwh > 0:
                ...
                # else: # No need for this else, trigger_repl remains False if conditions not met
                    # Handle case where base capacity is zero or None
                    # trigger_repl = False # This line was incorrect, it would reset trigger_repl if base_capacity_mwh > 0 but threshold wasn't met.
            ...
            if remaining_loan_balance > 0 and year <= loan_term:
                ...
                try:
                    ...
                    # Need to calculate payment based on *original* loan amount and term,
                    # but apply it to the *remaining* balance for interest/principal split.
                    # A proper amortization schedule is needed.
                    # Let's simplify and use the level payment calculated once,
                    # and then calculate interest/principal based on remaining balance.
                    # This is still an approximation without a full schedule.
                    ...
                    # Principal is the remainder of the level payment, capped by remaining balance
                    ...
                    # Fallback: Simple straight-line principal repayment + interest on declining balance
                    if loan_term > 0:
                         principal_payment_t = loan_amount / loan_term # Straight line principal
                         principal_payment_t = min(principal_payment_t, remaining_loan_balance) # Cap principal
                         interest_payment_t = remaining_loan_balance * loan_interest
                         annual_debt_payment = principal_payment_t + interest_payment_t
                         remaining_loan_balance -= principal_payment_t
                         if remaining_loan_balance < 0.01: remaining_loan_balance = 0.0
                    else: # Should not happen if loan_term > 0 check passes
                         principal_payment_t = 0.0
                         interest_payment_t = 0.0
                         annual_debt_payment = 0.0
        ...
        for i, row in enumerate(detailed_cash_flows):
            if i == 0 or row['Year'] > loan_term or (row['Interest Payment'] + row['Principal Payment']) <= 0: continue # Skip year 0, years after loan term, and years with no debt service
            ...
            # Proxy CFADS = Net Savings + Ancillary Revenue - O&M Cost + Depreciation # Add back depreciation as it's non-cash
            cfads_proxy = row['Net Savings'] + row['Ancillary Revenue'] - row['O&M Cost'] + row['Depreciation']
        ...
        return {
            ...
            # Estimate Yr 1 discharge - this calculation was incorrect.
            # Get it directly from the detailed cash flow for Year 1
            "total_annual_discharge_mwh_yr1": detailed_cash_flows[0].get('Gross Savings (Before Charging)', 0) / (utility_params.get(KEY_ENERGY_RATES,{}).get('peak',1)/1000) if len(detailed_cash_flows)>0 and utility_params.get(KEY_ENERGY_RATES,{}).get('peak',1) > 0 else 0 # Gross Savings / ($/MWh) -> MWh
            }
...
def create_technology_comparison_table(current_tech, capacity_mwh, power_mw):
    ...
    for tech in techs_to_compare:
        ...
        try:
            ...
            annual_om = 0.0
            # Use the correct O&M key based on the technology data structure
            ...
            # Fallback if neither key is explicitly in the template (shouldn't happen with current data)
            else: annual_om = (tech_data.get(KEY_FIXED_OM, 0) * power_kw) + (tech_data.get(KEY_OM_KWHR_YR, 0) * capacity_kwh)
                ...
                dbc.Accordion(id=ID_PARAM_ACCORDION, start_collapsed=False, always_open=True, children=[
                    ...
                    dbc.AccordionItem(title="Utility Rates & TOU", children=[
                        dbc.Card(className="p-3", children=[
                            ...
                            # REMOVED initial style={'display': 'none'} from ID_SEASONAL_CONTAINER
...
# MODIFIED: Change seasonal inputs from Input to State
@app.callback(
...
def update_utility_params_store(utility_provider, off_peak_rate, mid_peak_rate, peak_rate, demand_charge, seasonal_toggle_value,
    ...
    # Use the State values for seasonal parameters if enabled
    ...
    if isinstance(triggered_id, str) and triggered_id == ID_UTILITY_DROPDOWN: raw_tou_periods = params.get("tou_periods", default_utility_params[KEY_TOU_RAW]) # Use "tou_periods" from utility_rates data
    else:
        raw_tou_periods = []
...
# MODIFIED: Add new callback to control visibility
@app.callback(
...
def update_seasonal_rates_ui(toggle_value, selected_utility):
    # This callback now populates the children AND controls visibility
    ...
    # Get values from the utility data source
...
# MODIFIED: Change Opex inputs from Input to State
@app.callback(
...
def display_advanced_calculation_results(n_clicks, eaf_params, # REMOVED bess_params from here initially
    ...
    print(f"  UI Opex: FixedOM={ui_fixed_om}, OMKWhr={ui_om_kwhyr}")
    ...
    # Update with technology template values first (for keys not in UI, like example_product)
    tech_template = bess_technology_data.get(bess_params[KEY_TECH], {})
    for key, value in tech_template.items():
        if key not in bess_params: # Only add if not already set by UI
    ...
    try:
        ...
        def fmt_c(v, decimals=0): return f"${v:,.{decimals}f}" if pd.notna(v) and isinstance(v, (int, float)) and abs(v) < 1e15 else "N/A"
        ...
        summary_card = dbc.Card([
            ...
            dbc.CardBody(html.Table([
                ...
                html.Tr([html.Td("Levelized Cost of Storage (LCOS)"), html.Td(fmt_c(financial_metrics.get('lcos')) + " / MWh discharged")]),
        ...
        cost_breakdown_card = dbc.Card([
            ...
            dbc.CardBody(html.Table([
                ...
                html.Tr([html.Td("Estimated Annual Discharge (Yr 1)"), html.Td(fmt_n(financial_metrics.get('total_annual_discharge_mwh_yr1'), 0) + " MWh")]),
            ], className="table table-sm")),
        ], className="mb-4")
        ...
        cashflow_table = html.Div("No detailed cash flow data available.")
        if detailed_cf_data:
            df_cf = pd.DataFrame(detailed_cf_data)
            # Format columns for display
            for col in df_cf.columns:
                if 'Cost' in col or 'Savings' in col or 'Revenue' in col or 'Payment' in col or 'Income' in col or 'Taxes' in col or 'Cash Flow' in col or 'Investment' in col or 'Balance' in col:
                     df_cf[col] = df_cf[col].apply(lambda x: fmt_c(x, 0)) # Format currency
                elif 'IRR' in col or 'WACC' in col or 'Rate' in col:
                     df_cf[col] = df_cf[col].apply(fmt_p) # Format percentage
                elif 'Year' in col or 'Life' in col or 'Term' in col:
                     df_cf[col] = df_cf[col].apply(lambda x: fmt_n(x, 0)) # Format integer
                elif 'DSCR' in col:
                     df_cf[col] = df_cf[col].apply(lambda x: fmt_n(x, 2)) # Format DSCR
                elif 'Capacity (%)' in col:
                     df_cf[col] = df_cf[col].apply(lambda x: fmt_n(x, 1) + "%") # Format percentage with 1 decimal
                elif 'MWh' in col:
                     df_cf[col] = df_cf[col].apply(lambda x: fmt_n(x, 0)) # Format MWh
                ...
                dbc.Collapse(
                    dbc.Card(dbc.CardBody([
                        ...
                        dash_table.DataTable(
                            data=df_cf.to_dict('records'),
                            columns=[{"name": i, "id": i} for i in df_cf.columns],
                            page_size=10,
                            sort_action="native",
                        ...
                        # html.Button("Download CSV", id="btn-download-cashflow", className="mt-3 btn btn-secondary btn-sm"), # Replaced by export_format="csv"
                    ])),
    ...
    try:
        ...
        if opt_results and opt_results.get("best_capacity") is not None:
            ...
            if not all_results_df.empty:
                ...
                for col in ["Capacity (Mwh)", "Power (Mw)"]: # Corrected column names based on rename
                     if col in all_results_df.columns: all_results_df[col] = all_results_df[col].map("{:.1f}".format)
                for col in ["Project NPV ($)", "LCOS ($/MWh)", "Equity Invest ($)"]:
                     if col in all_results_df.columns: all_results_df[col] = all_results_df[col].apply(fmt_c)
                ...
                if "Payback (Yrs)" in all_results_df.columns: all_results_df["Payback (Yrs)"] = all_results_df["Payback (Yrs)"].apply(fmt_y)
                ...
                all_results_table = dash_table.DataTable(
                    ...
                    style_cell_conditional=[{'if': {'column_id': c}, 'textAlign': 'left'} for c in ["Capacity (MWh)", "Power (MW)"]], # Corrected column names
                )
...
@app.callback(
    [# Utility Outputs
     ...
     # Add outputs for the seasonal inputs themselves
     ...
     # Add outputs for the Opex inputs themselves
