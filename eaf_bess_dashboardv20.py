...
from dash import (
    ...
    DiskcacheManager,
    CeleryManager,
)  # For potential background tasks later
...
def clean_state_data(obj):
    """Recursively converts numpy types and timestamps in nested dicts/lists."""
    if isinstance(obj, dict):
        return {k: clean_state_data(v) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [clean_state_data(elem) for elem in obj]
    elif isinstance(obj, np.integer):
        return int(obj)
    elif isinstance(obj, np.floating):
        return float(obj)
    elif isinstance(obj, np.ndarray):
        return clean_state_data(obj.tolist())  # Convert numpy arrays to lists first
    elif isinstance(obj, (datetime, pd.Timestamp)):  # Handle datetime/timestamps
        # Check if it's timezone-aware and convert appropriately if needed
        # For simplicity, assuming naive or converting to ISO format directly
        try:
            return obj.isoformat()
        except Exception:  # Fallback if isoformat fails for some reason
            return str(obj)
    # Add other potential non-serializable types if needed
    else:
        # Assume it's already a standard type (int, float, str, bool, None)
        return obj
...
def fmt_c(v, decimals=0):
    ...
    if (
        pd.notna(v) and isinstance(v, (int, float)) and abs(v) < 1e15
    ):  # Avoid formatting huge numbers
...
def calculate_financial_metrics_advanced(
    ...
    try:
        ...
        base_capacity_mwh = max(
            0.001, local_safe_float(bess_params.get(KEY_CAPACITY, 0.001))
        )
        base_power_mw = max(
            0.001, local_safe_float(bess_params.get(KEY_POWER_MAX, 0.001))
        )
        ...
        inflation_rate = local_safe_float(
            financial_params.get(KEY_INFLATION, 0.024), 0.024
        )
        tax_rate = local_safe_float(financial_params.get(KEY_TAX_RATE, 0.2009), 0.2009)
        ancillary_revenue_yr1 = local_safe_float(
            financial_params.get(KEY_ANCILLARY_REVENUE, 0.0), 0.0
        )
        years = max(1, years)
        ...
        loan_percent = (
            local_safe_float(debt_params.get(KEY_LOAN_PERCENT, 0.0), 0.0) / 100.0
        )
        ...
        degrad_cap_yr = (
            local_safe_float(degrad_params.get(KEY_DEGRAD_CAP_YR, 0.0), 0.0) / 100.0
        )
        degrad_rte_yr = (
            local_safe_float(degrad_params.get(KEY_DEGRAD_RTE_YR, 0.0), 0.0) / 100.0
        )
        repl_thresh_percent = local_safe_float(
            degrad_params.get(KEY_REPL_THRESH, 70.0), 70.0
        )
        ...
        base_calendar_life = local_safe_float(
            bess_params.get(KEY_CALENDAR_LIFE, 15), 15
        )
        ...
        itc_taken = incentive_results.get("breakdown", {}).get(
            "Investment Tax Credit (ITC)", 0.0
        )
        depreciable_basis = total_initial_cost - (
            0.5 * itc_taken if itc_taken > 0 else 0.0
        )

        # Loan Amount & Equity
        loan_amount = total_initial_cost * loan_percent
        equity_investment = (
            total_initial_cost - loan_amount - total_incentive
        )  # Equity needed after loan and incentives
        ...
        insurance_percent_yr = local_safe_float(
            bess_params.get(KEY_INSURANCE, 0.0), 0.0
        )
        ...
        if (
            KEY_OM_KWHR_YR in tech_template and om_cost_per_kwh_yr > 0
        ):  # Check if this key exists AND has a value in the template
            om_base_cost = om_cost_per_kwh_yr * base_capacity_kwh
        elif (
            KEY_FIXED_OM in tech_template and fixed_om_per_kw_yr > 0
        ):  # Check if this key exists AND has a value
            om_base_cost = fixed_om_per_kw_yr * base_power_kw
        else:  # Fallback if template is missing O&M keys or values are zero
            om_base_cost = (fixed_om_per_kw_yr * base_power_kw) + (
                om_cost_per_kwh_yr * base_capacity_kwh
            )  # Sum both if unsure
        ...
        disconnect_cost_per_kwh = local_safe_float(
            bess_params.get(KEY_DISCONNECT_COST, 0.0), 0.0
        )
        recycling_cost_per_kwh = local_safe_float(
            bess_params.get(KEY_RECYCLING_COST, 0.0), 0.0
        )
        decomm_cost_base = (
            disconnect_cost_per_kwh + recycling_cost_per_kwh
        ) * base_capacity_kwh

        # --- Initialize Loop Variables ---
        project_cash_flows = [
            -total_initial_cost + total_incentive
        ]  # Project CF: Start with net cost before loan
        equity_cash_flows = [
            -equity_investment
        ]  # Equity CF: Start with equity invested
        ...
        battery_age_years = 0
        replacement_scheduled_this_year = (
            False  # Flag to prevent multiple replacements in one year trigger
        )
        ...
        for year in range(1, years + 1):
            ...
            capacity_degradation_factor = max(
                0, 1.0 - (degrad_cap_yr * battery_age_years)
            )
            ...
            current_power_mw = (
                base_power_mw  # Assume power doesn't degrade (simplification)
            )
            ...
            bess_params_t[KEY_POWER_MAX] = (
                current_power_mw  # Could degrade power too if needed
            )
            ...
            billing_results_t = calculate_yearly_savings_discharge(
                eaf_params, bess_params_t, utility_params, year
            )
            savings_t = billing_results_t.get(
                "annual_savings", 0.0
            )  # Net savings after charging costs
            annual_discharge_t = billing_results_t.get(
                "total_annual_discharge_mwh", 0.0
            )
            ...
            dod_fraction = (
                local_safe_float(bess_params.get(KEY_DOD, 100.0), 100.0) / 100.0
            )
            dod_fraction = max(0.01, min(1.0, dod_fraction))  # Ensure DoD is valid
            equiv_cycles_t = (
                (annual_discharge_t / (base_capacity_mwh * dod_fraction))
                if (base_capacity_mwh * dod_fraction) > 0
                else 0.0
            )
            ...
            if (
                trigger_repl and not replacement_scheduled_this_year
            ):  # Avoid double counting if multiple triggers hit
                ...
                replacement_scheduled_this_year = (
                    True  # Flag that replacement happened this year
                )
            else:
            ...
            if remaining_loan_balance > 0 and year <= loan_term and loan_term > 0:
                ...
                try:
                    ...
                    original_loan_payment = (
                        npf.pmt(loan_interest, loan_term, loan_amount) * -1
                        if loan_term > 0
                        else 0
                    )
                    ...
                    print(
                        f"Warning: Could not use npf.pmt for amortization ({pmt_err}). Using simplified payment."
                    )
                    original_loan_payment = calculate_debt_payment(
                        loan_amount, loan_interest, loan_term
                    )
                ...
                principal_payment_t = max(
                    0, principal_payment_t
                )  # Ensure principal isn't negative if interest > payment
                principal_payment_t = min(
                    principal_payment_t, remaining_loan_balance
                ...
                if remaining_loan_balance < 0.01:
            ...
            # 7. Depreciation
            depreciation_t = calculate_macrs_depreciation(
                depreciable_basis, macrs_schedule_name, year
            )
            ...
            equity_cf_t = (
                (savings_t + ancillary_revenue_t - o_m_cost_t - interest_payment_t)
                * (1.0 - tax_rate)
            ...
            total_gross_costs_t = (
                ...
                + replacement_cost_year_gross
                + decommissioning_cost_gross
                + billing_results_t.get("annual_charging_cost", 0.0)
            )
            ...
            detailed_cash_flows.append(
                {
                    "Year": year,
                    "Gross Savings (Before Charging)": savings_t
                    + billing_results_t.get(
                        "annual_charging_cost", 0.0
                    ),  # Show savings before charging cost
                    ...
                    "Cumulative Equity Cash Flow": sum(
                        equity_cash_flows
                    ),  # Recalculate cumulative CF
                    "Remaining Loan Balance": remaining_loan_balance,
                    "BESS Capacity (%)": (
                        (current_capacity_mwh / base_capacity_mwh * 100.0)
                        if base_capacity_mwh > 0
                        else 0.0
                    ),
                }
        ...
        project_irr = (
            npf.irr(project_cash_flows)
            if project_cash_flows
            and project_cash_flows[0] < 0
            and any(cf > 0 for cf in project_cash_flows[1:])
            else float("nan")
        )
        equity_npv = (
            npf.npv(wacc, equity_cash_flows) if wacc > -1 else float("nan")
        )  # Discount equity CFs at WACC (or cost of equity if known)
        equity_irr = (
            npf.irr(equity_cash_flows)
            if equity_cash_flows
            and equity_cash_flows[0] < 0
            and any(cf > 0 for cf in equity_cash_flows[1:])
            else float("nan")
        )
        ...
        if equity_cash_flows:
            ...
            if (
            ...
            ):  # Payback is immediate if no investment or positive CF in year 0
                payback_years = 0.0
            else:
                ...
                for yr_pbk in range(1, len(equity_cash_flows)):
                    ...
                    if (
                        cumulative_equity_cf + cf_yr >= 0 and cf_yr > 0
                    ):  # Check if CF crosses zero and is positive
                        fraction = abs(cumulative_equity_cf) / cf_yr
                        payback_years = (yr_pbk - 1) + fraction
                        break
                    cumulative_equity_cf += cf_yr
                    if (
                        yr_pbk == len(equity_cash_flows) - 1
                        and cumulative_equity_cf < 0
                    ):
                        payback_years = float(
                            "inf"
                        )  # Never pays back if still negative at the end
        ...
        for row in detailed_cash_flows:  # Iterates Year 1 onwards
            if row["Year"] > loan_term:
                continue  # Only consider years during the loan term
            debt_service = row["Interest Payment"] + row["Principal Payment"]
            if debt_service <= 0:
                continue  # Skip years with no debt service
            ...
            )  # Subtract replacement cost as a proxy for capex needed before debt service
        ...
        if detailed_cash_flows:
            total_annual_discharge_mwh_yr1 = detailed_cash_flows[0].get(
                "Gross Savings (Before Charging)", 0.0
            )  # This is actually savings, need discharge
            # Re-calculate discharge for year 1 using the billing function
            billing_yr1 = calculate_yearly_savings_discharge(
                eaf_params, bess_params, utility_params, 1
            )  # Use non-degraded params for year 1
            total_annual_discharge_mwh_yr1 = billing_yr1.get(
                "total_annual_discharge_mwh", 0.0
            )
        ...
        return {
            ...
            "net_initial_cost": total_initial_cost
            - total_incentive,  # Net cost before loan
        ...
        return {  # Return default structure on error
            "project_npv": float("nan"),
            "project_irr": float("nan"),
            "equity_npv": float("nan"),
            ...
            "total_annual_discharge_mwh_yr1": 0,
        }


# --- Optimization Function (Uses Advanced Metrics) ---
def optimize_battery_size_advanced(
    ...
    for capacity in capacity_options:
        for power in power_options:
            ...
            try:
                ...
                metrics = calculate_financial_metrics_advanced(
                    test_bess_params,
                    financial_params,
                    eaf_params,
                    utility_params,
    ...
    return {
        ...
        f"best_{metric_to_optimize}": best_metric_val,
    ...
    children=[
        ...
        dbc.Row(
            [
                dbc.Col(
                    html.H1(
                        "Advanced Battery Profitability Tool", className="text-center"
                    ),
                    width=10,
                ),
                dbc.Col(
                        ...
                        dbc.Container(
                            children=[
                                ...
                                html.Div(
                                    [
                                        html.Label(
                                            "Mill Selection:", className="form-label"
                                        ),
                                        dcc.Dropdown(
                                            id=ID_MILL_DROPDOWN,
                                            options=[
                ...
                dbc.Tab(
                    ...
                    children=[
                        dbc.Container(
                            children=[
                                html.H3(
                                    "System Parameters", className="my-4 text-center"
                                ),
                                dbc.Accordion(
                                    id=ID_PARAM_ACCORDION,
                                    start_collapsed=False,
                                    always_open=True,
                                    children=[
                                        # --- Utility Accordion Item ---
                                        dbc.AccordionItem(
                                            title="Utility Rates & TOU",
                                            children=[
                                                dbc.Card(
                                                    className="p-3",
                                                    children=[
                                                        ...
                                                        html.P(
                                                            "Define periods covering 24 hours. Gaps filled with Off-Peak.",
                                            ...
                                            children=[
                                                dbc.Card(
                                                    ...
                                                    children=[
                                                            ...
                                                            [
                                                                dbc.Col(
                                                                    create_bess_input_group(
                                                                        ...
                                                                        "MWh",
                                                                        tooltip_text="Total energy the BESS can store.",
                                                                    ),
                                                                    md=6,
                                                                ),
                                                                dbc.Col(
                                                                    create_bess_input_group(
                                                                        ...
                                                                        "MW",
                                                                        min_val=0.1,
                                                                        tooltip_text="Maximum rate of charge/discharge.",
                                                                    ),
                                                                    md=6,
                                                                ),
                                                            ]
                                                        ...
                                                        html.Div(
                                                            id=ID_BESS_C_RATE_DISPLAY,
                                                            className="mt-1 mb-3 text-muted small text-center",
                                                        ),
                                                        html.Div(
                                                            [
                                                                dbc.Label(
                                                                    "Select BESS Technology:",
                                                                    html_for=ID_BESS_TECH_DROPDOWN,
                                                                ),
                                                                dcc.Dropdown(
                                                                    ...
                                                                    options=[
                                                                        ...
                                                                        for tech in bess_technology_data.keys()
                                                                    ],
                                                                    value=default_bess_params_store[
                                                                        KEY_TECH
                                                                    ],
                                                                    clearable=False,
                                                                    className="mb-1",
                                                                            ...
                                                                            dbc.CardBody(
                                                                                [
                                                                                    create_bess_input_group(
                                                                                        ...
                                                                                        "$/kWh",
                                                                                        tooltip_text="Storage Block + Balance of System cost per kWh.",
                                                                                    ),
                                                                                    create_bess_input_group(
                                                                                        "PCS Cost:",
                                                                                        ID_BESS_PCS_COST,
                                                                                        ...
                                                                                        "$/kW",
                                                                                        tooltip_text="Power Conversion System cost per kW.",
                                                                                    ),
                                                                                    create_bess_input_group(
                                                                                        "System Integration:",
                                                                                        ID_BESS_SYS_INT_COST,
                                                                                        default_bess_params_store[
                                                                                            KEY_SYS_INT_COST
                                                                                        ],
                                                                                        "$/kWh",
                                                                                        tooltip_text="Engineering, Software, Commissioning cost per kWh.",
                                                                                    ),
                                                                                    create_bess_input_group(
                                                                                        "EPC Cost:",
                                                                                        ID_BESS_EPC_COST,
                                                                                        ...
                                                                                        "$/kWh",
                                                                                        tooltip_text="Engineering, Procurement, Construction cost per kWh.",
                                                                                    ),
                                                                                ]
                                                                            ),
                                                                        ],
                                                                                ...
                                                                                dbc.CardBody(
                                                                                    [
                                                                                        ...
                                                                                        create_bess_input_group(
                                                                                            ...
                                                                                            "%",
                                                                                            min_val=1,
                                                                                            tooltip_text="AC-to-AC efficiency for a full charge/discharge cycle.",
                                                                                        ),
                                                                                        create_bess_input_group(
                                                                                            "Insurance Rate:",
                                                                                            ID_BESS_INSURANCE,
                                                                                            ...
                                                                                            "%/yr",
                                                                                            step=0.01,
                                                                                            tooltip_text="Annual insurance cost as a percentage of initial capital cost.",
                                                                                        ),
                                                                                    ]
                                                                                ),
                                                                            ],
                                                                            ...
                                                                            [
                                                                                ...
                                                                                dbc.CardBody(
                                                                                    [
                                                                                        ...
                                                                                        create_bess_input_group(
                                                                                            "Recycling/Disposal:",
                                                                                            ID_BESS_RECYCLING_COST,
                                                                                            default_bess_params_store[
                                                                                                KEY_RECYCLING_COST
                                                                                            ],
                                                                                            "$/kWh",
                                                                                            min_val=None,
                                                                                            tooltip_text="Net cost (can be negative).",
                                                                                        ),  # Allow negative
                                                                                    ]
                                                                                ),
                                                                            ]
                                                                            ...
                                                                            [
                                                                                dbc.Col(
                                                                                    create_bess_input_group(
                                                                                        ...
                                                                                        "cycles",
                                                                                        min_val=100,
                                                                                        tooltip_text="Cycles until end-of-life (e.g., 80% capacity).",
                                                                                    ),
                                                                                    md=6,
                                                                                ),
                                                                                dbc.Col(
                                                                                    create_bess_input_group(
                                                                                        "Base Calendar Life:",
                                                                                        ID_BESS_CALENDAR_LIFE,
                                                                                        default_bess_params_store[
                                                                                            KEY_CALENDAR_LIFE
                                                                                        ],
                                                                                        "years",
                                                                                        min_val=1,
                                                                                        tooltip_text="Expected lifespan based on time.",
                                                                                    ),
                                                                                    md=6,
                                                                                ),
                                                                            ]
                                                                            ...
                                                                            [
                                                                                dbc.Col(
                                                                                    create_bess_input_group(
                                                                                        ...
                                                                                        min_val=1,
                                                                                        max_val=100,
                                                                                        tooltip_text="Recommended max discharge per cycle.",
                                                                                    ),
                                                                                    md=6,
                                                                                ),
                                                                                dbc.Col(
                                                                                    create_bess_input_group(
                                                                                        "Capacity Degrad Rate:",
                                                                                        ID_DEGRAD_RATE_CAP_YR,
                                                                                        default_financial_params[
                                                                                            KEY_DEGRAD_PARAMS
                                                                                        ][
                                                                                            KEY_DEGRAD_CAP_YR
                                                                                        ...
                                                                                        min_val=0,
                                                                                        step=0.1,
                                                                                        tooltip_text="Annual capacity loss.",
                                                                                    ),
                                                                                    md=6,
                                                                                ),
                                                                            ]
                                                                            ...
                                                                            [
                                                                                ...
                                                                                dbc.Col(
                                                                                    create_bess_input_group(
                                                                                        ...
                                                                                        min_val=0,
                                                                                        max_val=100,
                                                                                        tooltip_text="Replace when capacity drops below this % of original.",
                                                                                    ),
                                                                                    md=6,
                                                                                ),
                                                                            ]
                                                        ...
                                                        dbc.Row(
                                                            [
                                                                dbc.Col(
                                                                    [
                                                                        html.Label(
                                                                            "Ancillary Revenue ($/yr):"
                                                                        ),
                                                                        dcc.Input(
                                                                            id=ID_ANCILLARY_REVENUE_INPUT,
                                                                            type="number",
                                                                            value=default_financial_params[
                                                                                KEY_ANCILLARY_REVENUE
                                                                            ],
                                                                            min=0,
                                                                            className="form-control form-control-sm",
                                                                        ),
                                                                ...
                                                                dbc.Col(
                                                                    [
                                                                        ...
                                                                        html.P(
                                                                            "Note: Less critical with decomm.",
                                                                            className="small text-muted",
                                                                        ),
                                                                    ],
                                                                    md=6,
                                                                            ...
                                                                            dbc.CardBody(
                                                                                [
                                                                                    create_bess_input_group(
                                                                                        ...
                                                                                        min_val=0,
                                                                                        max_val=100,
                                                                                        tooltip_text="Percentage of Initial Gross Cost financed.",
                                                                                    ),
                                                                                    create_bess_input_group(
                                                                                        "Interest Rate:",
                                                                                        ID_LOAN_INTEREST_RATE,
                                                                                        default_financial_params[
                                                                                            KEY_DEBT_PARAMS
                                                                                        ][
                                                                                            KEY_LOAN_INTEREST
                                                                                        ...
                                                                                        * 100,
                                                                                        ...
                                                                                        min_val=0,
                                                                                        step=0.1,
                                                                                        tooltip_text="Annual loan interest rate.",
                                                                                    ),
                                                                                    create_bess_input_group(
                                                                                        "Loan Term:",
                                                                                        ID_LOAN_TERM_YEARS,
                                                                                        ...
                                                                                        min_val=1,
                                                                                        step=1,
                                                                                        tooltip_text="Loan repayment period.",
                                                                                    ),
                                                                                ]
                                                                            ),
                                                                        ]
                                                                        ...
                                                                        [
                                                                            ...
                                                                            dbc.CardBody(
                                                                                [
                                                                                    ...
                                                                                    dbc.Tooltip(
                                                                                        "Modified Accelerated Cost Recovery System (US Tax Depreciation). Select the appropriate schedule for energy storage property.",
                                                                                        target=ID_MACRS_SCHEDULE,
                                                                                    ),
                                                                                ]
                                                                            ),
                                                                        ]
                                                            ...
                                                            [
                                                                ...
                                                                html.Div(
                                                                    [
                                                                        dbc.Label(
                                                                            "Bonus Percentage (%):",
                                                                            html_for="bonus-credit-percentage",
                                                                            size="sm",
                                                                        ),
                                                                        dcc.Input(
                                                    ...
                                                    children=[  # Custom
                                                        ...
                                                        html.Div(
                                                            [
                                                                dbc.Label(
                                                                    "Incentive Type:",
                                                                    html_for="custom-incentive-type",
                                                                    size="sm",
                                                                ),
                                                                dcc.RadioItems(
                                                        ...
                                                        html.Div(
                                                            [
                                                                dbc.Label(
                                                                    "Incentive Amount:",
                                                                    html_for="custom-incentive-amount",
                                                                    size="sm",
                                                                ),
                                                                dcc.Input(
                                                        ...
                                                        html.Div(
                                                            [
                                                                dbc.Label(
                                                                    "Description:",
                                                                    html_for="custom-incentive-description",
                                                                    size="sm",
                                                                ),
                                                                dcc.Input(
                        ...
                        dbc.Container(
                            children=[
                                dcc.Loading(
                                    ...
                                    children=[
                                        html.Div(
                                            id=ID_RESULTS_OUTPUT, className="mt-4"
                            ...
                            fluid=True,
                            className="mt-4",
                        )  # Use fluid container for results
                    ],
                ),  # End Tab 4
                # Optimization Tab
                dbc.Tab(
                    label="5. Battery Sizing Tool",
                    tab_id="tab-optimization",
                    children=[
                        dbc.Container(
                            children=[
                                dcc.Loading(
                                    ...
                                    children=[
                                        html.Div(
                                            id=ID_OPTIMIZE_OUTPUT, className="mt-4"
                                        )  # Populated by callback
                                    ],
                                )
                            ],
...
@app.callback(
    ...
    [
        ...
        State("winter-multiplier", "value"),
        State("summer-multiplier", "value"),
        State("shoulder-multiplier", "value"),
...
def update_params_from_mill(selected_mill):
    ...
    off_peak = utility_data.get(KEY_ENERGY_RATES, {}).get(
        "off_peak", default_utility_params[KEY_ENERGY_RATES]["off_peak"]
    )
    mid_peak = utility_data.get(KEY_ENERGY_RATES, {}).get(
        "mid_peak", default_utility_params[KEY_ENERGY_RATES]["mid_peak"]
    )
    peak = utility_data.get(KEY_ENERGY_RATES, {}).get(
        "peak", default_utility_params[KEY_ENERGY_RATES]["peak"]
    )
    demand = utility_data.get(
        KEY_DEMAND_CHARGE, default_utility_params[KEY_DEMAND_CHARGE]
    )
    seasonal_enabled = (
        ["enabled"]
        if utility_data.get(KEY_SEASONAL, default_utility_params[KEY_SEASONAL])
        else []
    )
    ...
    cycles = mill_data.get(
        KEY_CYCLES_PER_DAY, nucor_mills["Custom"][KEY_CYCLES_PER_DAY]
    )
    duration = mill_data.get(
        KEY_CYCLE_DURATION, nucor_mills["Custom"][KEY_CYCLE_DURATION]
    )
    ...
    if triggered_input == ID_ADD_TOU_BTN:
        ...
        if new_row_elements:
            ...
            try:
                ...
                print(
                    "Error updating index for new TOU row components"
                )  # Handle potential structure issues
...
@app.callback(
    ...
    ],  # Control calc error visibility too
    [
    ...
    ],  # Use data stores for validation
    prevent_initial_call=True,
)
def validate_inputs_advanced(
    ...
    if not utility_params or not isinstance(utility_params, dict):
        ...
        if safe_float(rates.get("off_peak")) is None or rates.get("off_peak") < 0:
            errors.append("Invalid Off-Peak Rate.")
        if safe_float(rates.get("mid_peak")) is None or rates.get("mid_peak") < 0:
            errors.append("Invalid Mid-Peak Rate.")
        if safe_float(rates.get("peak")) is None or rates.get("peak") < 0:
            errors.append("Invalid Peak Rate.")
        if (
            safe_float(utility_params.get(KEY_DEMAND_CHARGE)) is None
            or utility_params.get("KEY_DEMAND_CHARGE") < 0
        ):
...
def update_bess_inputs_from_technology(selected_technology):
    ...
    om_kwhyr_input_group = create_bess_input_group(
        ...
        "$/kWh/yr",
        tooltip_text="Annual O&M cost per kWh discharged or per kWh capacity (check units).",
...
def update_bess_params_store(
    ...
    # Corrected helper functions defined inside this callback:
    def safe_float(value, default=0.0):
        try:
            # Attempt conversion only if value is not None
            if value is not None:
                # Handle potential empty strings from inputs
                if isinstance(value, str) and value.strip() == "":
                    return default
                return float(value)
            else:
                # Value is None, return the default
                return default
        except (ValueError, TypeError):
...
def display_advanced_calculation_results(
    ...
    try:
        ...
        if detailed_cf_data:
            ...
            df_cf_display = (
                df_cf.copy()
            )  # Keep original data for potential export if needed
...
def display_advanced_optimization_results(
    n_clicks,
    eaf_params,  # REMOVED bess_base_params from store
    utility_params,
    financial_params,
    incentive_params,
    validation_is_open,
            ...
            if not all_results_df.empty:
                ...
                all_results_table = dash_table.DataTable(
                    ...
                    style_cell_conditional=[
                        ...
                        for c in ["Capacity (MWh)", "Power (MW)"]  # Use new names
                    ],
                )
...
@app.callback(
    [
        ...
        Output(
            ID_CALCULATION_ERR, "children", allow_duplicate=True
        ),  # For load status/errors
        Output(ID_CALCULATION_ERR, "is_open", allow_duplicate=True),
    ],
...
@app.callback(
    [  # Utility Outputs
        ...
        Output(ID_DAYS_PER_YEAR, "value", allow_duplicate=True),
        # BESS Outputs
        Output(ID_BESS_CAPACITY, "value", allow_duplicate=True),
        Output(ID_BESS_POWER, "value", allow_duplicate=True),
        Output(ID_BESS_TECH_DROPDOWN, "value", allow_duplicate=True),
        Output(ID_BESS_SB_BOS_COST, "value", allow_duplicate=True),
        Output(ID_BESS_PCS_COST, "value", allow_duplicate=True),
        Output(ID_BESS_EPC_COST, "value", allow_duplicate=True),
        Output(
            ID_BESS_SYS_INT_COST, "value", allow_duplicate=True
        ),  # Opex container updated via tech dropdown callback trigger below
        Output(ID_BESS_RTE, "value", allow_duplicate=True),
        Output(ID_BESS_INSURANCE, "value", allow_duplicate=True),
        Output(ID_BESS_DISCONNECT_COST, "value", allow_duplicate=True),
        Output(ID_BESS_RECYCLING_COST, "value", allow_duplicate=True),
        Output(ID_BESS_CYCLE_LIFE, "value", allow_duplicate=True),
        Output(ID_BESS_DOD, "value", allow_duplicate=True),
        Output(ID_BESS_CALENDAR_LIFE, "value", allow_duplicate=True),
        # Financial Outputs
        Output(ID_WACC, "value", allow_duplicate=True),
        Output(ID_LIFESPAN, "value", allow_duplicate=True),
        Output(ID_TAX_RATE, "value", allow_duplicate=True),
        Output(ID_INFLATION_RATE, "value", allow_duplicate=True),
        Output(ID_SALVAGE, "value", allow_duplicate=True),
        Output(ID_ANCILLARY_REVENUE_INPUT, "value", allow_duplicate=True),
        Output(ID_LOAN_AMOUNT_PERCENT, "value", allow_duplicate=True),
        Output(ID_LOAN_INTEREST_RATE, "value", allow_duplicate=True),
        Output(ID_LOAN_TERM_YEARS, "value", allow_duplicate=True),
        Output(ID_MACRS_SCHEDULE, "value", allow_duplicate=True),
        Output(ID_DEGRAD_RATE_CAP_YR, "value", allow_duplicate=True),
        Output(ID_DEGRAD_RATE_RTE_YR, "value", allow_duplicate=True),
        Output(ID_REPLACEMENT_THRESHOLD, "value", allow_duplicate=True),
        # Incentive Outputs (Checklists need list values)
        Output(ID_ITC_ENABLED, "value", allow_duplicate=True),
        Output(ID_ITC_PERCENT, "value", allow_duplicate=True),
        Output("ceic-enabled", "value", allow_duplicate=True),
        Output("ceic-percentage", "value", allow_duplicate=True),
        Output("bonus-credit-enabled", "value", allow_duplicate=True),
        Output("bonus-credit-percentage", "value", allow_duplicate=True),
        Output("sgip-enabled", "value", allow_duplicate=True),
        Output("sgip-amount", "value", allow_duplicate=True),
        Output("ess-enabled", "value", allow_duplicate=True),
        Output("ess-amount", "value", allow_duplicate=True),
        Output("mabi-enabled", "value", allow_duplicate=True),
        Output("mabi-amount", "value", allow_duplicate=True),
        Output("cs-enabled", "value", allow_duplicate=True),
        Output("cs-amount", "value", allow_duplicate=True),
        Output("custom-incentive-enabled", "value", allow_duplicate=True),
        Output("custom-incentive-type", "value", allow_duplicate=True),
        Output("custom-incentive-amount", "value", allow_duplicate=True),
        Output("custom-incentive-description", "value", allow_duplicate=True),
        # Add outputs for the seasonal inputs themselves (needed because they are inside a callback-generated div)
        Output("winter-multiplier", "value", allow_duplicate=True),
        Output("summer-multiplier", "value", allow_duplicate=True),
        Output("shoulder-multiplier", "value", allow_duplicate=True),
        Output("winter-months", "value", allow_duplicate=True),
        Output("summer-months", "value", allow_duplicate=True),
        Output("shoulder-months", "value", allow_duplicate=True),
        # Add outputs for the Opex inputs themselves (needed because they are inside a callback-generated div)
        Output(ID_BESS_FIXED_OM, "value", allow_duplicate=True),
        Output(ID_BESS_OM_KWHR_YR, "value", allow_duplicate=True),
        # Add outputs for the error display (Total 60 UI + 2 Error = 62 Outputs)
...
def update_ui_from_loaded_state(
    ...
    num_ui_outputs = 60  # Define the number of UI outputs excluding the error outputs

    # Clean the data read from State stores immediately
    try:
        print("Attempting to clean loaded state data...")  # Optional: for debugging
        eaf_data = clean_state_data(eaf_data)
        bess_data = clean_state_data(bess_data)
        util_data = clean_state_data(util_data)
        fin_data = clean_state_data(fin_data)
        inc_data = clean_state_data(inc_data)
        print("State data cleaned successfully.")  # Optional: for debugging
    except Exception as e:
        print(f"ERROR cleaning state data: {e}")
        error_msg = dbc.Alert(f"Error processing stored data: {e}", color="danger")
        # Return no_update for all UI elements + error message
        return ([dash.no_update] * num_ui_outputs) + [error_msg, True]

    # Basic check if data exists after cleaning
    if load_trigger is None or not all(
        [eaf_data, bess_data, util_data, fin_data, inc_data]
    ):
        return ([dash.no_update] * num_ui_outputs) + [
            dash.no_update,
            dash.no_update,
        ]  # No update, no error message

    print("Validating and updating UI elements from loaded state...")

    # --- Data Validation ---
    error_messages = []
    # Validate Utility Data Structure more thoroughly
    if not isinstance(util_data, dict):
        error_messages.append("Stored Utility Data Error: Data is not a dictionary.")
    else:
        energy_rates_dict = util_data.get(KEY_ENERGY_RATES)
        if not isinstance(energy_rates_dict, dict):
            error_messages.append(
                "Stored Utility Data Error: 'energy_rates' is missing or not a dictionary."
            )
        else:
            # Check for essential rate keys
            if "off_peak" not in energy_rates_dict:
                error_messages.append(
                    "Stored Utility Data Error: 'off_peak' rate is missing."
                )
            if "mid_peak" not in energy_rates_dict:
                error_messages.append(
                    "Stored Utility Data Error: 'mid_peak' rate is missing."
                )
            if "peak" not in energy_rates_dict:
                error_messages.append(
                    "Stored Utility Data Error: 'peak' rate is missing."
                )
        # Check for other essential utility keys
        if KEY_DEMAND_CHARGE not in util_data:
            error_messages.append(
                "Stored Utility Data Error: Demand charge is missing."
            )
        if KEY_TOU_RAW not in util_data:
            error_messages.append("Stored Utility Data Error: Raw TOU periods missing.")
        if KEY_SEASONAL not in util_data:
            error_messages.append("Stored Utility Data Error: Seasonal flag missing.")
        # Add more checks as needed for other stores (eaf_data, bess_data, fin_data, inc_data)

    if error_messages:
        print(f"ERROR: Incomplete data found in stored state: {error_messages}")
        alert_content = [html.H5("Data Integrity Issue", className="text-danger")]
        alert_content.extend([html.P(msg) for msg in error_messages])
        alert_content.append(
            html.P(
                "Cannot update UI or run calculations accurately. Please check parameters or load a valid project file."
            )
        )
        error_alert = dbc.Alert(alert_content, color="danger", dismissable=True)
        # Return no_update for all UI elements + the error message
        return ([dash.no_update] * num_ui_outputs) + [error_alert, True]
    # --- End Data Validation ---

    # --- If validation passes, proceed with extracting values SAFELY ---
    try:
        # Utility UI (using .get with defaults as fallback after validation)
        util_provider = (
            "Custom Utility"  # Default - consider improving matching logic if needed
        )
        # Match logic placeholder - implement robust matching if required
        # for name, data in utility_rates.items():
        #     if ... : util_provider = name; break
        energy_rates_dict = util_data.get(
            KEY_ENERGY_RATES, {}
        )  # Already validated to be dict
        off_peak = energy_rates_dict.get("off_peak", 0)
        mid_peak = energy_rates_dict.get("mid_peak", 0)
        peak_val = energy_rates_dict.get("peak", 0)  # Renamed to avoid conflict
        ...
        winter_mult_ui = util_data.get(KEY_WINTER_MULT, 1.0)
        summer_mult_ui = util_data.get(KEY_SUMMER_MULT, 1.0)
        shoulder_mult_ui = util_data.get(KEY_SHOULDER_MULT, 1.0)
        winter_m_ui = ",".join(map(str, util_data.get(KEY_WINTER_MONTHS, [])))
        summer_m_ui = ",".join(map(str, util_data.get(KEY_SUMMER_MONTHS, [])))
        shoulder_m_ui = ",".join(map(str, util_data.get(KEY_SHOULDER_MONTHS, [])))
        ...
        cycle_dur = eaf_data.get(
            KEY_CYCLE_DURATION_INPUT, eaf_data.get(KEY_CYCLE_DURATION, 0)
        )  # Prefer input tracker
        ...
        loan_int_ui = (
            fin_data.get(KEY_DEBT_PARAMS, {}).get(KEY_LOAN_INTEREST, 0) * 100.0
        )
        ...
        custom_en_ui = (
            ["enabled"] if inc_data.get("custom_incentive_enabled", False) else []
        )
        ...
        # --- Return all values IN ORDER to update the UI ---
        # (Ensure this list exactly matches the order and number of Outputs: 60 UI + 2 Error)
        ui_updates = [
            util_provider,
            off_peak,
            mid_peak,
        ...
        # Return UI updates + clear the error message
        return ui_updates + [None, False]

    except Exception as e:
        # Catch any unexpected error during value extraction
        print(f"ERROR extracting validated state data for UI update: {e}")
        error_msg = dbc.Alert(
            f"Error preparing UI update from stored data: {e}", color="danger"
        )
        return ([dash.no_update] * num_ui_outputs) + [error_msg, True]
